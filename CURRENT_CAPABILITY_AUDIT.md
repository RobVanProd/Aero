...
Audit date: 2026-08-02; latest corrective checkpoint: 2026-08-13

Current accepted public master:
protected CAP-023 merge and its accepted tree, with the exact acceptance
identities and current alert state recorded in the CAP-023 section below.

### CAP-023 accepted: fixed-shape exact-i32 ReLU-and-argmax inference product

Accepted CAP-023 is a zero-production product/evidence checkpoint for one
source-embedded fixed-shape exact-`i32` CPU inference application.
...
Accepted CAP-021 is a zero-production product/evidence checkpoint for one
source-embedded flat-record two-stage exact-`i32` CPU scoring application.
...
remains the immutable result-composition checkpoint, and accepted CAP-019 remains the
latest compiler/profile capability widening. CAP-019 closes only the bounded initialized-local,
...
CAP-014 remains the exact-array profile origin; accepted CAP-019 remains the latest
compiler/profile capability widening, with CAP-018 retained as its immutable-result
...
The post-CAP-023 order begins with an accepted-head inference
correctness/reproducibility/artifact-footprint evidence gate with no performance claim:
...
higher means lower implementation risk or lower evidence cost. Rank 1 is a
zero-production accepted-head correctness, reproducibility, and artifact-footprint
evidence gate; it permits no compiler/profile change, benchmark timing, or performance
claim. Rank 2 remains exact CPU plus recursive-CopyData application-profile composition
readiness and a task-local red probe only, not implementation. Rank 3 remains quantized
numerical-kernel readiness and a task-local red probe only, not implementation.
...
Accepted CAP-007 supplies one library-owned authority that prepares a checked program
through lexing, fatal parsing, direct-module
...
checked admission rejects; the accepted path now rejects that product consistently before
...
`-O0`/`-O2` retain exact output/exit 91. Exact candidate
`bfb7adb36af4c2d5bef65d615a2f8e32bff7902b`
and protected PR #35 merge `5a64acaffa5e7f7167823861a45bc49c6bb670b4`
share tree `3ab515666d310a7e72f28347ecd2561297ce0eca`. Candidate CI/Rust CI/CodeQL
`31519719961`/`31519753426`, `31519753421`, and `31519751604` pass; exact
merge-head CI `31520069468`, Rust CI `31520069315`, and CodeQL `31520068887`
also pass. CAP-007 adds no source syntax or semantics and does not make the selected
language subset stable. It closes the canonical Milestone 0 trusted-entrypoint and
artifact-eligibility mechanism; stable-subset classification remains open.
...
rewrites contextual annotations, constructors, and bound patterns, then reuses the
accepted owned-enum, exhaustive-Match, checked-IR, independent-verifier, and LLVM
paths. No runtime polymorphism, boxing, generic-enum ABI, or second enum engine is
introduced.

The representative application executes both `Sample<Reading<int>>` and
`Sample<char>` through nongeneric transport and exhaustive `Match` at unchanged exact
...
- `CORE-082` is accepted public at implementation commit `d0312e8`, tree
  ...
  `a0aff6d2b730fc6499cc08b0d6e1d85d484aeb47`. Bounded PR #6 passed all nine
  exact-head checks at `3edfa139`, merged as `962bb49f`, and passed all three
  post-merge master workflows. It admits only closed, exactly annotated primitive
  constants through one shared evaluator and substitutes them before checked IR.
  The complete 209-library/32-binary root gate, verifier controls, and pinned
  LLVM/Clang 22.1.8 native exit-81 proof pass. No storage/global, aggregate constant,
  cross-module lookup, general CTFE, layout, ABI, stability, or release conclusion
  follows.
