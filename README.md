...
| **Memory** | Shallow move tracking plus bounded, publicly accepted whole-place immutable and mutable references and direct reassignment over the exact admitted recursive CopyData universe. Accepted CORE-064–079 add bounded owned-enum replacement, joins, reinitialization, Match results, and loop ownership. Accepted CORE-083–086 add non-escaping mutable whole-place enum replacement, immutable and mutable enum-reference Match observation, and exact lexical-loan/CFG proof. Local candidate CORE-087 composes exactly one mutable whole-place reference parameter with independent recursive CopyData parameters in any declared position under one shared indexed contract. Projected writes/borrows, multiple reference parameters, partial moves, free enum dereference/transport, enum aggregate storage, and general aliasing remain unsupported. No general borrow checker, general mutable-reference model, lifetime analysis, drop model, stable pointer ABI, or memory-safety guarantee. Reference results remain unsupported. |
| **Data Types** | Recursive finite CopyData composition and bounded positional recursive CopyData owned enums—including exact variants with two or more fields—are publicly accepted with exhaustive identifier-bound Match, internal transport, exact mutable whole-owner replacement/reinitialization, acyclic conditional ownership joins, and fresh per-iteration loop-local owners. Accepted CORE-072 adds `char`; CORE-074–076 add typed Match results; and CORE-083–086 add bounded enum-reference replacement and observation plus homogeneous discarded `Void` Match results. Local candidate CORE-087 transports the same admitted CopyData values beside one mutable reference without adding a data topology. `print!` and `println!` are effect-only `Void`, not scalar values. Named-field/generic variants, wider patterns, enum aggregate storage, enum fields/arrays, free enum dereference or transport through references, projection, dynamic collections, unsupported/cyclic structs, and broader storage or destructuring semantics remain unsupported. |
...
| **Function calls** | Accepted CORE-068 centralizes exact named-call classification across both semantic paths and checked admission/lowering. Existing nongeneric functions over admitted scalar, recursive CopyData, owned-enum, and reference-parameter contracts remain supported; local candidate CORE-087 additionally composes exactly one mutable reference with independent CopyData side parameters in any declared position. Missing or unsupported signatures, wrong arguments, and `Void` value use fail before checked IR. Overloads, conversions, generic/trait/closure calls, multiple reference parameters, reference results, and stable callable ABI remain unsupported. |
...
> CORE-087 is a local candidate mixed-signature checkpoint. Exactly one admitted
...
> execute in the tracked native exit-87 specimen. The focused target passes 3/3, the
> verifier corruption control passes 1/1, and the exact root gate passes 215 library and
> 32 binary tests plus every integration target and doc tests; public gates remain
> pending. Multiple references, owner-dependent sides, projections, reference results
> or escape/storage/capture, lifetime/NLL/drop, stable ABI/FFI, accelerators, and
> general memory-safety claims remain excluded.
