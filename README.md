...
| **Function calls** | Local CORE-068 candidate centralizes exact named-call classification across both semantic paths and checked admission/lowering. Existing nongeneric functions over admitted scalar, recursive CopyData, owned-enum, and reference-parameter contracts remain supported; missing or unsupported signatures, wrong arguments, and `Void` value use fail before checked IR. Public candidate/LLVM 22 evidence is pending. Overloads, conversions, generic/trait/closure calls, reference results, and stable callable ABI remain unsupported. |
