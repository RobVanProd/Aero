...
| **Function calls** | Accepted CORE-068 centralizes exact named-call classification across both semantic paths and checked admission/lowering. Existing nongeneric functions over admitted scalar, recursive CopyData, owned-enum, and reference contracts remain supported; accepted CORE-087 composes one mutable reference with CopyData sides, and local CORE-088 composes exactly one mutable reference with one or more immutable references plus CopyData sides in every declared order. Missing or unsupported signatures, wrong arguments, alias overlap with the mutable source, and `Void` value use fail before checked IR. Overloads, conversions, generic/trait/closure calls, multiple mutable parameters, reference results, and stable callable ABI remain unsupported. |
...
> CORE-088 is a local candidate mixed exclusive/shared-reference checkpoint. Every
...
> control passes 1/1, the complete repository gate is green, and pinned local
> LLVM/Clang 22.1.8 executes the direct-module specimen through public and independent
> native paths at exit 88. Public gates remain pending. Multiple mutable parameters,
