...
| **Function calls** | Accepted CORE-068 centralizes exact named-call classification across both semantic paths and checked admission/lowering. Existing nongeneric functions over admitted scalar, recursive CopyData, owned-enum, and reference contracts remain supported; accepted CORE-087 composes one mutable reference with CopyData sides, CORE-088 adds immutable-reference sides, and CORE-089 admits two or more pairwise-disjoint mutable references with any immutable and CopyData sides in every declared order. CORE-090 permits already admitted calls as projected-assignment RHS values but does not add projected arguments. Accepted CAP-003 transports exact concrete recursive-CopyData `Option`/`Result` values, CAP-004 transports exact concrete user-defined generic CopyData structs through nongeneric internal parameters and results, CAP-005 specializes bound-free whole-value generic transport functions over exact recursive finite CopyData arguments, and CAP-006 transports exact specialized user-defined generic enums through nongeneric parameters/results. Missing or unsupported signatures, wrong arguments, mutable-root overlap with any other argument, and `Void` value use fail before checked IR. Overloads, conversions, trait-bounded or operational generic functions, generic-to-generic/recursive calls, general trait/closure calls, projected arguments, reference results, question-mark propagation, and stable callable ABI remain unsupported. |
...
> **Project status after CAP-006:** Aero remains a Minimal Prototype in correctness
> recovery, not a complete or stable language. Accepted public and compiler-capability
> baseline is protected CAP-006 merge
> `bdfd4f5a282043ee957c1bf03975e266de5b9b6c`.
...
> **CAP-007 local candidate (not accepted):** public artifact-free
...
> `aero test` false-success is closed locally. Focused 3/3 and the complete normalized
...
> accepted public master remains final CAP-006
> state `80f35b4` pending exact public and protected CAP-007 acceptance.
