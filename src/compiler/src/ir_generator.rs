...
mod tests {
    ...
    fn let_with_integer_emits_alloca_and_store() {
        ...
        assert!(main.iter().any(|i| matches!(i, crate::ir::Inst::Alloca(_, n) if n == "x")));
        assert!(main.iter().any(|i| matches!(i, crate::ir::Inst::Store(_, crate::ir::Value::ImmInt(1)))));
    }
    ...
    fn binary_expression_requires_type_annotation_in_ast() {
        ...
        let (val, ty) = ir_gen.generate_expression_ir(expr, &mut crate::ir::Function {
            name: "main".to_string(),
            body: vec![],
            next_reg: 0,
            next_ptr: 0,
        ...
        assert!(matches!(val, crate::ir::Value::ImmInt(3) | crate::ir::Value::Reg(_)));
    }
}
