...
"#,
    ...
    let case_results = cases
        ...
        .map(|(name, source, expected_success)| {
            evaluate_conformance_case(name, source, expected_success)
        })
        .collect::<Vec<_>>();
...
"#;
    ...
    let mut results = run_frontend_mechanized_checks(source);
...
mod tests {
    ...
    fn invalid_source_becomes_explicit_failed_cases_and_checks() {
        ...
        let lexical_case =
            evaluate_conformance_case("invalid_lexical_source", lexical_source, true);
        assert!(!lexical_case.passed);
        assert!(lexical_case.details.contains("Lex error"));

        let parse_source = "fn main() { let = ; }";
        let parse_case = evaluate_conformance_case("invalid_parse_source", parse_source, true);
