...
fn exact_i32_array_system_gate_is_anchored_on_linux_and_windows() {
    ...
    let required_inference_evidence: [(&str, &str, &[&str]); 2] = [
        (
            ...
            &[
                ...
                "inference_bitcode=\"${RUNNER_TEMP}/exact_i32_array_inference.linux.bc\"",
                "llvm-as-22 \"${llvm}\" -o \"${inference_bitcode}\"",
                "test -s \"${inference_bitcode}\"",
                "inference_call_count=\"$(grep -Fc 'call [8 x i32] @infer_record([20 x i32]' \"${llvm}\")\"",
                "test \"${inference_call_count}\" -eq 7",
                "inference_preservation_call_count=\"$(grep -Fc 'call i32 @records_equal([20 x i32]' \"${llvm}\")\"",
                "test \"${inference_preservation_call_count}\" -eq 7",
                "inference_result_call_count=\"$(grep -Fc 'call i32 @results_equal([8 x i32]' \"${llvm}\")\"",
                "test \"${inference_result_call_count}\" -eq 7",
                "inference_forbidden_pattern='double|fptosi|fptoui|sitofp|uitofp| nsw | nuw |icmp u(gt|ge|lt|le) i32|udiv i32|urem i32|lshr i32| x i8]|<[^>]* x [^>]*>|\\[[0-9]+ x \\[|extract(element|value)|insert(element|value)|shufflevector|aero_quant|quantization'",
                "if grep -Eq \"${inference_forbidden_pattern}\" \"${llvm}\"; then",
                "inference_decode_llvm=\"$(awk '",
                "/^define \\[8 x i32\\] @infer_record\\(\\[20 x i32\\] %aero\\.arg\\.record\\) \\{$/ { inference_decode_capture = 1; inference_decode_definitions++ }",
                "inference_decode_capture { print }",
                "inference_decode_capture && /^}$/ { inference_decode_capture = 0 }",
                "END { if (inference_decode_definitions != 1) exit 1 }",
                "inference_matvec_2x3_llvm=\"$(awk '",
                "/^define \\[2 x i32\\] @matvec_2x3\\(\\[6 x i32\\] %aero\\.arg\\.matrix, \\[3 x i32\\] %aero\\.arg\\.vector\\) \\{$/ { inference_matvec_2x3_capture = 1; inference_matvec_2x3_definitions++ }",
                "inference_matvec_2x3_capture { print }",
                "inference_matvec_2x3_capture && /^}$/ { inference_matvec_2x3_capture = 0 }",
                "END { if (inference_matvec_2x3_definitions != 1) exit 1 }",
                "inference_matvec_2x2_llvm=\"$(awk '",
                "/^define \\[2 x i32\\] @matvec_2x2\\(\\[4 x i32\\] %aero\\.arg\\.matrix, \\[2 x i32\\] %aero\\.arg\\.vector\\) \\{$/ { inference_matvec_2x2_capture = 1; inference_matvec_2x2_definitions++ }",
                "inference_matvec_2x2_capture { print }",
                "inference_matvec_2x2_capture && /^}$/ { inference_matvec_2x2_capture = 0 }",
                "END { if (inference_matvec_2x2_definitions != 1) exit 1 }",
                "inference_matvec_2x3_call_count=\"$(grep -Fc 'call [2 x i32] @matvec_2x3([6 x i32]' \"${inference_decode_llvm}\")\"",
                "test \"${inference_matvec_2x3_call_count}\" -eq 1",
                "inference_matvec_2x2_call_count=\"$(grep -Fc 'call [2 x i32] @matvec_2x2([4 x i32]' \"${inference_decode_llvm}\")\"",
                "test \"${inference_matvec_2x2_call_count}\" -eq 1",
                "inference_decode_pattern='(?ms)^",
                "inference_decode_count=\"$(grep -Pzo -- \"${inference_decode_pattern}\" <(printf '%s\\n' \"${inference_decode_llvm}\") | tr -cd '\\000' | wc -c)\"",
                "test \"${inference_decode_count}\" -eq 1",
                "inference_decode_chain_pattern='(?ms)^",
                "inference_decode_chain_count=\"$(grep -Pzo -- \"${inference_decode_chain_pattern}\" <(printf '%s\\n' \"${inference_decode_llvm}\") | tr -cd '\\000' | wc -c)\"",
                "test \"${inference_decode_chain_count}\" -eq 6",
                "for inference_payload in 3:3 6:6 12:2 14:4 18:2; do",
                "IFS=: read -r inference_payload_offset inference_payload_width <<< \"${inference_payload}\"",
                "inference_payload_offset_pattern=\"(?ms)^",
                "(?<inference_payload_record_base>%ptr[0-9]+) = alloca \\[20 x i32\\], align 8",
                "store \\[20 x i32\\] %aero\\.arg\\.record, \\[20 x i32\\]\\* \\k<inference_payload_record_base>, align 8",
                "(?<inference_payload_initial>%reg[0-9]+) = load \\[${inference_payload_width} x i32\\]",
                "(?<inference_payload_base>%ptr[0-9]+) = alloca \\[${inference_payload_width} x i32\\], align 8",
                "store \\[${inference_payload_width} x i32\\] \\k<inference_payload_initial>, \\[${inference_payload_width} x i32\\]\\* \\k<inference_payload_base>, align 8",
                "(?<inference_payload_dst_index>%reg[0-9]+) = load i32, i32\\* (?<inference_payload_index_slot>%ptr[0-9]+), align 4",
                "(?<inference_payload_dst_upper>%reg[0-9]+) = icmp slt i32 \\k<inference_payload_dst_index>, ${inference_payload_width}",
                "(?<inference_payload_destination>%ptr[0-9]+) = getelementptr inbounds \\[${inference_payload_width} x i32\\], \\[${inference_payload_width} x i32\\]\\* \\k<inference_payload_base>, i64 0, i64 \\k<inference_payload_dst_extended>",
                "(?<inference_payload_source_index>%reg[0-9]+) = load i32, i32\\* \\k<inference_payload_index_slot>, align 4",
                "(?<inference_payload_index>%reg[0-9]+) = add i32 ${inference_payload_offset}, \\k<inference_payload_source_index>",
                "inference_payload_offset_count=\"$(grep -Pzo -- \"${inference_payload_offset_pattern}\" <(printf '%s\\n' \"${inference_decode_llvm}\") | tr -cd '\\000' | wc -c)\"",
                "test \"${inference_payload_offset_count}\" -eq 1",
                "inference_header_pattern='(?ms)^",
                "inference_header_count=\"$(grep -Pzo -- \"${inference_header_pattern}\" <(printf '%s\\n' \"${inference_decode_llvm}\") | tr -cd '\\000' | wc -c)\"",
                "test \"${inference_header_count}\" -eq 1",
                "inference_relu_pattern='(?ms)^",
                "inference_relu_count=\"$(grep -Pzo -- \"${inference_relu_pattern}\" <(printf '%s\\n' \"${inference_decode_llvm}\") | tr -cd '\\000' | wc -c)\"",
                "test \"${inference_relu_count}\" -eq 1",
                "inference_logit_pattern='(?ms)^",
                "inference_logit_count=\"$(grep -Pzo -- \"${inference_logit_pattern}\" <(printf '%s\\n' \"${inference_decode_llvm}\") | tr -cd '\\000' | wc -c)\"",
                "test \"${inference_logit_count}\" -eq 1",
                "inference_argmax_pattern='(?ms)^",
                "inference_argmax_count=\"$(grep -Pzo -- \"${inference_argmax_pattern}\" <(printf '%s\\n' \"${inference_decode_llvm}\") | tr -cd '\\000' | wc -c)\"",
                "test \"${inference_argmax_count}\" -eq 1",
                "inference_matvec_2x3_pattern='(?ms)^",
                "inference_matvec_2x3_count=\"$(grep -Pzo -- \"${inference_matvec_2x3_pattern}\" <(printf '%s\\n' \"${inference_matvec_2x3_llvm}\") | tr -cd '\\000' | wc -c)\"",
                "test \"${inference_matvec_2x3_count}\" -eq 1",
                "inference_matvec_2x2_pattern='(?ms)^",
                "inference_matvec_2x2_count=\"$(grep -Pzo -- \"${inference_matvec_2x2_pattern}\" <(printf '%s\\n' \"${inference_matvec_2x2_llvm}\") | tr -cd '\\000' | wc -c)\"",
                "test \"${inference_matvec_2x2_count}\" -eq 1",
                "inference_guard_pattern='(?m)^",
                "inference_guard_count=\"$(grep -Pzo -- \"${inference_guard_pattern}\" <(printf '%s\\n' \"${inference_decode_llvm}\") | tr -cd '\\000' | wc -c)\"",
                "test \"${inference_guard_count}\" -eq 18",
                "inference_matvec_2x3_guard_count=\"$(grep -Pzo -- \"${inference_guard_pattern}\" <(printf '%s\\n' \"${inference_matvec_2x3_llvm}\") | tr -cd '\\000' | wc -c)\"",
                "test \"${inference_matvec_2x3_guard_count}\" -eq 3",
                "inference_matvec_2x2_guard_count=\"$(grep -Pzo -- \"${inference_guard_pattern}\" <(printf '%s\\n' \"${inference_matvec_2x2_llvm}\") | tr -cd '\\000' | wc -c)\"",
                "test \"${inference_matvec_2x2_guard_count}\" -eq 3",
            ],
        ),
        (
            "Windows",
            ...
            &[
                ...
                "$inferenceMatvec2x3FunctionMatches.Count -ne 1",
                ...
                "$inferenceMatvec2x2FunctionMatches.Count -ne 1",
                ...
                "$inferencePayloadOffsetPattern = \"(?ms)^",
                "(?<inference_payload_record_base>%ptr[0-9]+) = alloca \\[20 x i32\\], align 8",
                "store \\[20 x i32\\] %aero\\.arg\\.record, \\[20 x i32\\]\\* \\k<inference_payload_record_base>, align 8",
                "(?<inference_payload_initial>%reg[0-9]+) = load \\[$inferencePayloadWidth x i32\\]",
                "(?<inference_payload_base>%ptr[0-9]+) = alloca \\[$inferencePayloadWidth x i32\\], align 8",
                "store \\[$inferencePayloadWidth x i32\\] \\k<inference_payload_initial>, \\[$inferencePayloadWidth x i32\\]\\* \\k<inference_payload_base>, align 8",
                "(?<inference_payload_dst_index>%reg[0-9]+) = load i32, i32\\* (?<inference_payload_index_slot>%ptr[0-9]+), align 4",
                "(?<inference_payload_dst_upper>%reg[0-9]+) = icmp slt i32 \\k<inference_payload_dst_index>, $inferencePayloadWidth",
                "(?<inference_payload_destination>%ptr[0-9]+) = getelementptr inbounds \\[$inferencePayloadWidth x i32\\], \\[$inferencePayloadWidth x i32\\]\\* \\k<inference_payload_base>, i64 0, i64 \\k<inference_payload_dst_extended>",
                "(?<inference_payload_source_index>%reg[0-9]+) = load i32, i32\\* \\k<inference_payload_index_slot>, align 4",
                "(?<inference_payload_index>%reg[0-9]+) = add i32 $inferencePayloadOffset, \\k<inference_payload_source_index>",
                "$inferencePayloadOffsetMatches = [regex]::Matches($inferenceDecodeText, $inferencePayloadOffsetPattern)",
                "$inferencePayloadOffsetMatches.Count -ne 1",
    ...
    let shared_inference_anchors = [
        ...
        "(?<inference_decode_index>%reg[0-9]+) = load i32, i32\\* (?<inference_decode_index_slot>%ptr[0-9]+), align 4",
        "(?<inference_decode_lower>%reg[0-9]+) = icmp sge i32 \\k<inference_decode_index>, 0",
        "(?<inference_decode_upper>%reg[0-9]+) = icmp slt i32 \\k<inference_decode_index>, (?<inference_decode_bound>[2346])",
        "(?<inference_decode_inbounds>%reg[0-9]+) = and i1 \\k<inference_decode_lower>, \\k<inference_decode_upper>",
        "br i1 \\k<inference_decode_inbounds>, label %aero\\.bounds\\.safe\\.(?<inference_decode_place>[0-9]+), label %aero\\.bounds\\.trap\\.\\k<inference_decode_place>",
        "aero\\.bounds\\.trap\\.\\k<inference_decode_place>:\\r?\\n  call void @llvm\\.trap\\(\\)\\r?\\n  unreachable\\r?\\naero\\.bounds\\.safe\\.\\k<inference_decode_place>:",
        "(?<inference_decode_extended>%reg[0-9]+) = sext i32 \\k<inference_decode_index> to i64",
        "(?<inference_decode_destination>%ptr[0-9]+) = getelementptr inbounds (?<inference_decode_aggregate>\\[\\k<inference_decode_bound> x i32\\]), \\k<inference_decode_aggregate>\\* %ptr[0-9]+, i64 0, i64 \\k<inference_decode_extended>",
        "(?<inference_record_source_index>%reg[0-9]+) = load i32, i32\\* \\k<inference_decode_index_slot>, align 4",
        "(?<inference_record_index>%reg[0-9]+) = add i32 (?<inference_record_offset>3|6|12|14|18), \\k<inference_record_source_index>)?",
        "(?<inference_record_lower>%reg[0-9]+) = icmp sge i32 (?(inference_record_index)\\k<inference_record_index>|\\k<inference_record_source_index>), 0",
        "(?<inference_record_upper>%reg[0-9]+) = icmp slt i32 (?(inference_record_index)\\k<inference_record_index>|\\k<inference_record_source_index>), 20",
        "(?<inference_record_inbounds>%reg[0-9]+) = and i1 \\k<inference_record_lower>, \\k<inference_record_upper>",
        "br i1 \\k<inference_record_inbounds>, label %aero\\.bounds\\.safe\\.(?<inference_record_place>[0-9]+), label %aero\\.bounds\\.trap\\.\\k<inference_record_place>",
        "aero\\.bounds\\.trap\\.\\k<inference_record_place>:\\r?\\n  call void @llvm\\.trap\\(\\)\\r?\\n  unreachable\\r?\\naero\\.bounds\\.safe\\.\\k<inference_record_place>:",
        "(?<inference_record_extended>%reg[0-9]+) = sext i32 (?(inference_record_index)\\k<inference_record_index>|\\k<inference_record_source_index>) to i64",
        "(?<inference_record_target>%ptr[0-9]+) = getelementptr inbounds \\[20 x i32\\], \\[20 x i32\\]\\* %ptr[0-9]+, i64 0, i64 \\k<inference_record_extended>",
        "(?<inference_record_value>%reg[0-9]+) = load i32, i32\\* \\k<inference_record_target>, align 4",
        "store i32 \\k<inference_record_value>, i32\\* \\k<inference_decode_destination>, align 4",
        "(?<inference_payload_dst_lower>%reg[0-9]+) = icmp sge i32 \\k<inference_payload_dst_index>, 0",
        "(?<inference_payload_dst_inbounds>%reg[0-9]+) = and i1 \\k<inference_payload_dst_lower>, \\k<inference_payload_dst_upper>",
        "br i1 \\k<inference_payload_dst_inbounds>, label %aero\\.bounds\\.safe\\.(?<inference_payload_dst_place>[0-9]+), label %aero\\.bounds\\.trap\\.\\k<inference_payload_dst_place>",
        "aero\\.bounds\\.trap\\.\\k<inference_payload_dst_place>:\\r?\\n  call void @llvm\\.trap\\(\\)\\r?\\n  unreachable\\r?\\naero\\.bounds\\.safe\\.\\k<inference_payload_dst_place>:",
        "(?<inference_payload_dst_extended>%reg[0-9]+) = sext i32 \\k<inference_payload_dst_index> to i64",
        "(?<inference_payload_lower>%reg[0-9]+) = icmp sge i32 \\k<inference_payload_index>, 0",
        "(?<inference_payload_upper>%reg[0-9]+) = icmp slt i32 \\k<inference_payload_index>, 20",
        "(?<inference_payload_inbounds>%reg[0-9]+) = and i1 \\k<inference_payload_lower>, \\k<inference_payload_upper>",
        "br i1 \\k<inference_payload_inbounds>, label %aero\\.bounds\\.safe\\.(?<inference_payload_place>[0-9]+), label %aero\\.bounds\\.trap\\.\\k<inference_payload_place>",
        "aero\\.bounds\\.trap\\.\\k<inference_payload_place>:\\r?\\n  call void @llvm\\.trap\\(\\)\\r?\\n  unreachable\\r?\\naero\\.bounds\\.safe\\.\\k<inference_payload_place>:",
        "(?<inference_payload_extended>%reg[0-9]+) = sext i32 \\k<inference_payload_index> to i64",
        "(?<inference_payload_record_target>%ptr[0-9]+) = getelementptr inbounds \\[20 x i32\\], \\[20 x i32\\]\\* \\k<inference_payload_record_base>, i64 0, i64 \\k<inference_payload_extended>",
        "(?<inference_payload_value>%reg[0-9]+) = load i32, i32\\* \\k<inference_payload_record_target>, align 4",
        "store i32 \\k<inference_payload_value>, i32\\* \\k<inference_payload_destination>, align 4",
        "(?<inference_header_record_base>%ptr[0-9]+) = alloca \\[20 x i32\\], align 8",
        "store \\[20 x i32\\] %aero\\.arg\\.record, \\[20 x i32\\]\\* \\k<inference_header_record_base>, align 8",
        "(?<inference_header_initial>%reg[0-9]+) = load \\[3 x i32\\]",
        "(?<inference_header_base>%ptr[0-9]+) = alloca \\[3 x i32\\], align 8",
        "store \\[3 x i32\\] \\k<inference_header_initial>, \\[3 x i32\\]\\* \\k<inference_header_base>, align 8",
        "(?<inference_header_decode_index>%reg[0-9]+) = load i32, i32\\* (?<inference_header_index_slot>%ptr[0-9]+), align 4",
        "(?<inference_header_decode_lower>%reg[0-9]+) = icmp sge i32 \\k<inference_header_decode_index>, 0",
        "(?<inference_header_decode_upper>%reg[0-9]+) = icmp slt i32 \\k<inference_header_decode_index>, 3",
        "(?<inference_header_decode_inbounds>%reg[0-9]+) = and i1 \\k<inference_header_decode_lower>, \\k<inference_header_decode_upper>",
        "br i1 \\k<inference_header_decode_inbounds>, label %aero\\.bounds\\.safe\\.(?<inference_header_decode_place>[0-9]+), label %aero\\.bounds\\.trap\\.\\k<inference_header_decode_place>",
        "aero\\.bounds\\.trap\\.\\k<inference_header_decode_place>:\\r?\\n  call void @llvm\\.trap\\(\\)\\r?\\n  unreachable\\r?\\naero\\.bounds\\.safe\\.\\k<inference_header_decode_place>:",
        "(?<inference_header_decode_extended>%reg[0-9]+) = sext i32 \\k<inference_header_decode_index> to i64",
        "(?<inference_header_destination>%ptr[0-9]+) = getelementptr inbounds \\[3 x i32\\], \\[3 x i32\\]\\* \\k<inference_header_base>, i64 0, i64 \\k<inference_header_decode_extended>",
        "(?<inference_header_source_index>%reg[0-9]+) = load i32, i32\\* \\k<inference_header_index_slot>, align 4",
        "(?<inference_header_source_lower>%reg[0-9]+) = icmp sge i32 \\k<inference_header_source_index>, 0",
        "(?<inference_header_source_upper>%reg[0-9]+) = icmp slt i32 \\k<inference_header_source_index>, 20",
        "(?<inference_header_source_inbounds>%reg[0-9]+) = and i1 \\k<inference_header_source_lower>, \\k<inference_header_source_upper>",
        "br i1 \\k<inference_header_source_inbounds>, label %aero\\.bounds\\.safe\\.(?<inference_header_source_place>[0-9]+), label %aero\\.bounds\\.trap\\.\\k<inference_header_source_place>",
        "aero\\.bounds\\.trap\\.\\k<inference_header_source_place>:\\r?\\n  call void @llvm\\.trap\\(\\)\\r?\\n  unreachable\\r?\\naero\\.bounds\\.safe\\.\\k<inference_header_source_place>:",
        "(?<inference_header_source_extended>%reg[0-9]+) = sext i32 \\k<inference_header_source_index> to i64",
        "(?<inference_header_source_target>%ptr[0-9]+) = getelementptr inbounds \\[20 x i32\\], \\[20 x i32\\]\\* \\k<inference_header_record_base>, i64 0, i64 \\k<inference_header_source_extended>",
        "(?<inference_header_source_value>%reg[0-9]+) = load i32, i32\\* \\k<inference_header_source_target>, align 4",
        "store i32 \\k<inference_header_source_value>, i32\\* \\k<inference_header_destination>, align 4",
        "(?<inference_header_lane0_target>%ptr[0-9]+) = getelementptr inbounds \\[3 x i32\\], \\[3 x i32\\]\\* \\k<inference_header_base>, i64 0, i64 0",
        "(?<inference_header_lane0>%reg[0-9]+) = load i32, i32\\* \\k<inference_header_lane0_target>, align 4",
        "(?<inference_header_lane0_is_two>%reg[0-9]+) = icmp eq i32 \\k<inference_header_lane0>, 2",
        "(?<inference_header_lane1_target>%ptr[0-9]+) = getelementptr inbounds \\[3 x i32\\], \\[3 x i32\\]\\* \\k<inference_header_base>, i64 0, i64 1",
        "(?<inference_header_lane1>%reg[0-9]+) = load i32, i32\\* \\k<inference_header_lane1_target>, align 4",
        "(?<inference_header_lane1_is_three>%reg[0-9]+) = icmp eq i32 \\k<inference_header_lane1>, 3",
        "(?<inference_header_first_pair>%reg[0-9]+) = and i1 \\k<inference_header_lane0_is_two>, \\k<inference_header_lane1_is_three>",
        "(?<inference_header_lane2_target>%ptr[0-9]+) = getelementptr inbounds \\[3 x i32\\], \\[3 x i32\\]\\* \\k<inference_header_base>, i64 0, i64 2",
        "(?<inference_header_lane2>%reg[0-9]+) = load i32, i32\\* \\k<inference_header_lane2_target>, align 4",
        "(?<inference_header_lane2_is_two>%reg[0-9]+) = icmp eq i32 \\k<inference_header_lane2>, 2",
        "(?<inference_header_gate>%reg[0-9]+) = and i1 \\k<inference_header_first_pair>, \\k<inference_header_lane2_is_two>",
        "br i1 \\k<inference_header_gate>, label %(?<inference_valid_path>if_then_[0-9]+), label %(?<inference_invalid_path>if_else_[0-9]+)",
        "\\k<inference_valid_path>:",
        "br label %(?<inference_result_merge>if_end_[0-9]+)\\r?\\n\\k<inference_invalid_path>:",
        "\\k<inference_invalid_path>:\\r?\\n  br label %\\k<inference_result_merge>",
        "\\k<inference_result_merge>:\\r?\\n  (?<inference_result>%reg[0-9]+) = load \\[8 x i32\\]",
        "ret \\[8 x i32\\] \\k<inference_result>",
        "(?<inference_relu_index>%reg[0-9]+) = load i32, i32\\* (?<inference_relu_index_slot>%ptr[0-9]+), align 4",
        "(?<inference_relu_raw_extended>%reg[0-9]+) = sext i32 \\k<inference_relu_index> to i64",
        "(?<inference_relu_raw_target>%ptr[0-9]+) = getelementptr inbounds \\[2 x i32\\], \\[2 x i32\\]\\* %ptr[0-9]+, i64 0, i64 \\k<inference_relu_raw_extended>",
        "(?<inference_relu_raw>%reg[0-9]+) = load i32, i32\\* \\k<inference_relu_raw_target>, align 4",
        "(?<inference_relu_bias_index>%reg[0-9]+) = load i32, i32\\* \\k<inference_relu_index_slot>, align 4",
        "(?<inference_relu_bias_extended>%reg[0-9]+) = sext i32 \\k<inference_relu_bias_index> to i64",
        "(?<inference_relu_bias_target>%ptr[0-9]+) = getelementptr inbounds \\[2 x i32\\], \\[2 x i32\\]\\* %ptr[0-9]+, i64 0, i64 \\k<inference_relu_bias_extended>",
        "(?<inference_relu_bias>%reg[0-9]+) = load i32, i32\\* \\k<inference_relu_bias_target>, align 4",
        "(?<inference_relu_sum>%reg[0-9]+) = add i32 \\k<inference_relu_raw>, \\k<inference_relu_bias>",
        "(?<inference_relu_home>%ptr[0-9]+) = alloca i32, align 4",
        "store i32 \\k<inference_relu_sum>, i32\\* \\k<inference_relu_home>, align 4",
        "(?<inference_relu_value>%reg[0-9]+) = load i32, i32\\* \\k<inference_relu_home>, align 4",
        "(?<inference_relu_positive>%reg[0-9]+) = icmp sgt i32 \\k<inference_relu_value>, 0",
        ...
        "br i1 \\k<inference_relu_positive>, label %(?<inference_relu_true>if_then_[0-9]+), label %(?<inference_relu_false>if_else_[0-9]+)",
        "\\k<inference_relu_true>:\\r?\\n  (?<inference_hidden_index>%reg[0-9]+) = load i32, i32\\* \\k<inference_relu_index_slot>, align 4",
        "(?<inference_hidden_extended>%reg[0-9]+) = sext i32 \\k<inference_hidden_index> to i64",
        "(?<inference_hidden_destination>%ptr[0-9]+) = getelementptr inbounds \\[2 x i32\\], \\[2 x i32\\]\\* %ptr[0-9]+, i64 0, i64 \\k<inference_hidden_extended>",
        "(?<inference_relu_stored_value>%reg[0-9]+) = load i32, i32\\* \\k<inference_relu_home>, align 4",
        "store i32 \\k<inference_relu_stored_value>, i32\\* \\k<inference_hidden_destination>, align 4",
        "store i32 \\k<inference_relu_stored_value>, i32\\* \\k<inference_hidden_destination>, align 4\\r?\\n  br label %(?<inference_relu_merge>if_end_[0-9]+)",
        "\\k<inference_relu_false>:\\r?\\n  br label %\\k<inference_relu_merge>",
        "(?<inference_logit_index>%reg[0-9]+) = load i32, i32\\* (?<inference_logit_index_slot>%ptr[0-9]+), align 4",
        "(?<inference_logit_dst_extended>%reg[0-9]+) = sext i32 \\k<inference_logit_index> to i64",
        "(?<inference_logit_destination>%ptr[0-9]+) = getelementptr inbounds \\[2 x i32\\], \\[2 x i32\\]\\* %ptr[0-9]+, i64 0, i64 \\k<inference_logit_dst_extended>",
        "(?<inference_raw_logit_index>%reg[0-9]+) = load i32, i32\\* \\k<inference_logit_index_slot>, align 4",
        "(?<inference_raw_logit_extended>%reg[0-9]+) = sext i32 \\k<inference_raw_logit_index> to i64",
        "(?<inference_raw_logit_target>%ptr[0-9]+) = getelementptr inbounds \\[2 x i32\\], \\[2 x i32\\]\\* %ptr[0-9]+, i64 0, i64 \\k<inference_raw_logit_extended>",
        "(?<inference_raw_logit>%reg[0-9]+) = load i32, i32\\* \\k<inference_raw_logit_target>, align 4",
        "(?<inference_second_bias_index>%reg[0-9]+) = load i32, i32\\* \\k<inference_logit_index_slot>, align 4",
        "(?<inference_second_bias_extended>%reg[0-9]+) = sext i32 \\k<inference_second_bias_index> to i64",
        "(?<inference_second_bias_target>%ptr[0-9]+) = getelementptr inbounds \\[2 x i32\\], \\[2 x i32\\]\\* %ptr[0-9]+, i64 0, i64 \\k<inference_second_bias_extended>",
        "(?<inference_second_bias>%reg[0-9]+) = load i32, i32\\* \\k<inference_second_bias_target>, align 4",
        "(?<inference_logit_sum>%reg[0-9]+) = add i32 \\k<inference_raw_logit>, \\k<inference_second_bias>",
        "store i32 \\k<inference_logit_sum>, i32\\* \\k<inference_logit_destination>, align 4",
        "(?<inference_logit_sum>%reg[0-9]+) = add i32 \\k<inference_raw_logit>, \\k<inference_second_bias>\\r?\\n  store i32 \\k<inference_logit_sum>, i32\\* \\k<inference_logit_destination>, align 4",
        "(?<inference_class_home>%ptr[0-9]+) = alloca i32, align 4",
        "store i32 0, i32\\* \\k<inference_class_home>, align 4",
        "(?<inference_class_home>%ptr[0-9]+) = alloca i32, align 4\\r?\\n  store i32 0, i32\\* \\k<inference_class_home>, align 4",
        "(?<inference_logit1_target>%ptr[0-9]+) = getelementptr inbounds \\[2 x i32\\], \\[2 x i32\\]\\* (?<inference_logits_base>%ptr[0-9]+), i64 0, i64 1",
        "(?<inference_logit1>%reg[0-9]+) = load i32, i32\\* \\k<inference_logit1_target>, align 4",
        "(?<inference_logit0_target>%ptr[0-9]+) = getelementptr inbounds \\[2 x i32\\], \\[2 x i32\\]\\* \\k<inference_logits_base>, i64 0, i64 0",
        "(?<inference_logit0>%reg[0-9]+) = load i32, i32\\* \\k<inference_logit0_target>, align 4",
        "(?<inference_argmax>%reg[0-9]+) = icmp sgt i32 \\k<inference_logit1>, \\k<inference_logit0>",
        "br i1 \\k<inference_argmax>, label %(?<inference_class_one>if_then_[0-9]+), label %(?<inference_class_zero>if_else_[0-9]+)",
        "\\k<inference_class_one>:\\r?\\n  store i32 1, i32\\* \\k<inference_class_home>, align 4\\r?\\n  br label %(?<inference_class_merge>if_end_[0-9]+)",
        "\\k<inference_class_zero>:\\r?\\n  br label %\\k<inference_class_merge>",
        "(?<inference_mv23_matrix_base>%ptr[0-9]+) = alloca \\[6 x i32\\], align 8",
        "store \\[6 x i32\\] %aero\\.arg\\.matrix, \\[6 x i32\\]\\* \\k<inference_mv23_matrix_base>, align 8",
        "(?<inference_mv23_vector_base>%ptr[0-9]+) = alloca \\[3 x i32\\], align 8",
        "store \\[3 x i32\\] %aero\\.arg\\.vector, \\[3 x i32\\]\\* \\k<inference_mv23_vector_base>, align 8",
        "(?<inference_mv23_output_base>%ptr[0-9]+) = alloca \\[2 x i32\\], align 8",
        "store \\[2 x i32\\] %reg[0-9]+, \\[2 x i32\\]\\* \\k<inference_mv23_output_base>, align 8",
        "(?<inference_mv23_row_slot>%ptr[0-9]+) = alloca i32, align 4",
        "(?<inference_mv23_column_slot>%ptr[0-9]+) = alloca i32, align 4",
        "(?<inference_mv23_accumulator_slot>%ptr[0-9]+) = alloca i32, align 4",
        "store i32 0, i32\\* \\k<inference_mv23_accumulator_slot>, align 4",
        "(?<inference_mv23_accumulator>%reg[0-9]+) = load i32, i32\\* \\k<inference_mv23_accumulator_slot>, align 4",
        "(?<inference_mv23_row>%reg[0-9]+) = load i32, i32\\* \\k<inference_mv23_row_slot>, align 4",
        "(?<inference_mv23_row_times_three>%reg[0-9]+) = mul i32 \\k<inference_mv23_row>, 3",
        "(?<inference_mv23_column>%reg[0-9]+) = load i32, i32\\* \\k<inference_mv23_column_slot>, align 4",
        "(?<inference_mv23_linear>%reg[0-9]+) = add i32 \\k<inference_mv23_row_times_three>, \\k<inference_mv23_column>",
        "(?<inference_mv23_matrix_lower>%reg[0-9]+) = icmp sge i32 \\k<inference_mv23_linear>, 0",
        "(?<inference_mv23_matrix_upper>%reg[0-9]+) = icmp slt i32 \\k<inference_mv23_linear>, 6",
        "(?<inference_mv23_matrix_inbounds>%reg[0-9]+) = and i1 \\k<inference_mv23_matrix_lower>, \\k<inference_mv23_matrix_upper>",
        "br i1 \\k<inference_mv23_matrix_inbounds>, label %aero\\.bounds\\.safe\\.(?<inference_mv23_matrix_place>[0-9]+), label %aero\\.bounds\\.trap\\.\\k<inference_mv23_matrix_place>",
        "aero\\.bounds\\.trap\\.\\k<inference_mv23_matrix_place>:\\r?\\n  call void @llvm\\.trap\\(\\)\\r?\\n  unreachable\\r?\\naero\\.bounds\\.safe\\.\\k<inference_mv23_matrix_place>:",
        "(?<inference_mv23_matrix_extended>%reg[0-9]+) = sext i32 \\k<inference_mv23_linear> to i64",
        "(?<inference_mv23_matrix_target>%ptr[0-9]+) = getelementptr inbounds \\[6 x i32\\], \\[6 x i32\\]\\* \\k<inference_mv23_matrix_base>, i64 0, i64 \\k<inference_mv23_matrix_extended>",
        "(?<inference_mv23_matrix_value>%reg[0-9]+) = load i32, i32\\* \\k<inference_mv23_matrix_target>, align 4",
        "(?<inference_mv23_vector_index>%reg[0-9]+) = load i32, i32\\* \\k<inference_mv23_column_slot>, align 4",
        "(?<inference_mv23_vector_lower>%reg[0-9]+) = icmp sge i32 \\k<inference_mv23_vector_index>, 0",
        "(?<inference_mv23_vector_upper>%reg[0-9]+) = icmp slt i32 \\k<inference_mv23_vector_index>, 3",
        "(?<inference_mv23_vector_inbounds>%reg[0-9]+) = and i1 \\k<inference_mv23_vector_lower>, \\k<inference_mv23_vector_upper>",
        "br i1 \\k<inference_mv23_vector_inbounds>, label %aero\\.bounds\\.safe\\.(?<inference_mv23_vector_place>[0-9]+), label %aero\\.bounds\\.trap\\.\\k<inference_mv23_vector_place>",
        "aero\\.bounds\\.trap\\.\\k<inference_mv23_vector_place>:\\r?\\n  call void @llvm\\.trap\\(\\)\\r?\\n  unreachable\\r?\\naero\\.bounds\\.safe\\.\\k<inference_mv23_vector_place>:",
        "(?<inference_mv23_vector_extended>%reg[0-9]+) = sext i32 \\k<inference_mv23_vector_index> to i64",
        "(?<inference_mv23_vector_target>%ptr[0-9]+) = getelementptr inbounds \\[3 x i32\\], \\[3 x i32\\]\\* \\k<inference_mv23_vector_base>, i64 0, i64 \\k<inference_mv23_vector_extended>",
        "(?<inference_mv23_vector_value>%reg[0-9]+) = load i32, i32\\* \\k<inference_mv23_vector_target>, align 4",
        "(?<inference_mv23_product>%reg[0-9]+) = mul i32 \\k<inference_mv23_matrix_value>, \\k<inference_mv23_vector_value>",
        "(?<inference_mv23_sum>%reg[0-9]+) = add i32 \\k<inference_mv23_accumulator>, \\k<inference_mv23_product>",
        "store i32 \\k<inference_mv23_sum>, i32\\* \\k<inference_mv23_accumulator_slot>, align 4",
        "(?<inference_mv23_output_index>%reg[0-9]+) = load i32, i32\\* \\k<inference_mv23_row_slot>, align 4",
        "(?<inference_mv23_output_lower>%reg[0-9]+) = icmp sge i32 \\k<inference_mv23_output_index>, 0",
        "(?<inference_mv23_output_upper>%reg[0-9]+) = icmp slt i32 \\k<inference_mv23_output_index>, 2",
        "(?<inference_mv23_output_inbounds>%reg[0-9]+) = and i1 \\k<inference_mv23_output_lower>, \\k<inference_mv23_output_upper>",
        "br i1 \\k<inference_mv23_output_inbounds>, label %aero\\.bounds\\.safe\\.(?<inference_mv23_output_place>[0-9]+), label %aero\\.bounds\\.trap\\.\\k<inference_mv23_output_place>",
        "aero\\.bounds\\.trap\\.\\k<inference_mv23_output_place>:\\r?\\n  call void @llvm\\.trap\\(\\)\\r?\\n  unreachable\\r?\\naero\\.bounds\\.safe\\.\\k<inference_mv23_output_place>:",
        "(?<inference_mv23_output_extended>%reg[0-9]+) = sext i32 \\k<inference_mv23_output_index> to i64",
        "(?<inference_mv23_output_target>%ptr[0-9]+) = getelementptr inbounds \\[2 x i32\\], \\[2 x i32\\]\\* \\k<inference_mv23_output_base>, i64 0, i64 \\k<inference_mv23_output_extended>",
        "(?<inference_mv23_output_value>%reg[0-9]+) = load i32, i32\\* \\k<inference_mv23_accumulator_slot>, align 4",
        "store i32 \\k<inference_mv23_output_value>, i32\\* \\k<inference_mv23_output_target>, align 4",
        "(?<inference_mv22_matrix_base>%ptr[0-9]+) = alloca \\[4 x i32\\], align 8",
        "store \\[4 x i32\\] %aero\\.arg\\.matrix, \\[4 x i32\\]\\* \\k<inference_mv22_matrix_base>, align 8",
        "(?<inference_mv22_vector_base>(?!\\k<inference_mv22_matrix_base>)%ptr[0-9]+) = alloca \\[2 x i32\\], align 8",
        "store \\[2 x i32\\] %aero\\.arg\\.vector, \\[2 x i32\\]\\* \\k<inference_mv22_vector_base>, align 8",
        "(?<inference_mv22_output_base>(?!\\k<inference_mv22_matrix_base>)(?!\\k<inference_mv22_vector_base>)%ptr[0-9]+) = alloca \\[2 x i32\\], align 8",
        "store \\[2 x i32\\] %reg[0-9]+, \\[2 x i32\\]\\* \\k<inference_mv22_output_base>, align 8",
        "(?<inference_mv22_row_slot>%ptr[0-9]+) = alloca i32, align 4",
        "(?<inference_mv22_column_slot>%ptr[0-9]+) = alloca i32, align 4",
        "(?<inference_mv22_accumulator_slot>%ptr[0-9]+) = alloca i32, align 4",
        "store i32 0, i32\\* \\k<inference_mv22_accumulator_slot>, align 4",
        "(?<inference_mv22_accumulator>%reg[0-9]+) = load i32, i32\\* \\k<inference_mv22_accumulator_slot>, align 4",
        "(?<inference_mv22_row>%reg[0-9]+) = load i32, i32\\* \\k<inference_mv22_row_slot>, align 4",
        "(?<inference_mv22_row_times_two>%reg[0-9]+) = mul i32 \\k<inference_mv22_row>, 2",
        "(?<inference_mv22_column>%reg[0-9]+) = load i32, i32\\* \\k<inference_mv22_column_slot>, align 4",
        "(?<inference_mv22_linear>%reg[0-9]+) = add i32 \\k<inference_mv22_row_times_two>, \\k<inference_mv22_column>",
        "(?<inference_mv22_matrix_lower>%reg[0-9]+) = icmp sge i32 \\k<inference_mv22_linear>, 0",
        "(?<inference_mv22_matrix_upper>%reg[0-9]+) = icmp slt i32 \\k<inference_mv22_linear>, 4",
        "(?<inference_mv22_matrix_inbounds>%reg[0-9]+) = and i1 \\k<inference_mv22_matrix_lower>, \\k<inference_mv22_matrix_upper>",
        "br i1 \\k<inference_mv22_matrix_inbounds>, label %aero\\.bounds\\.safe\\.(?<inference_mv22_matrix_place>[0-9]+), label %aero\\.bounds\\.trap\\.\\k<inference_mv22_matrix_place>",
        "aero\\.bounds\\.trap\\.\\k<inference_mv22_matrix_place>:\\r?\\n  call void @llvm\\.trap\\(\\)\\r?\\n  unreachable\\r?\\naero\\.bounds\\.safe\\.\\k<inference_mv22_matrix_place>:",
        "(?<inference_mv22_matrix_extended>%reg[0-9]+) = sext i32 \\k<inference_mv22_linear> to i64",
        "(?<inference_mv22_matrix_target>%ptr[0-9]+) = getelementptr inbounds \\[4 x i32\\], \\[4 x i32\\]\\* \\k<inference_mv22_matrix_base>, i64 0, i64 \\k<inference_mv22_matrix_extended>",
        "(?<inference_mv22_matrix_value>%reg[0-9]+) = load i32, i32\\* \\k<inference_mv22_matrix_target>, align 4",
        "(?<inference_mv22_vector_index>%reg[0-9]+) = load i32, i32\\* \\k<inference_mv22_column_slot>, align 4",
        "(?<inference_mv22_vector_lower>%reg[0-9]+) = icmp sge i32 \\k<inference_mv22_vector_index>, 0",
        "(?<inference_mv22_vector_upper>%reg[0-9]+) = icmp slt i32 \\k<inference_mv22_vector_index>, 2",
        "(?<inference_mv22_vector_inbounds>%reg[0-9]+) = and i1 \\k<inference_mv22_vector_lower>, \\k<inference_mv22_vector_upper>",
        "br i1 \\k<inference_mv22_vector_inbounds>, label %aero\\.bounds\\.safe\\.(?<inference_mv22_vector_place>[0-9]+), label %aero\\.bounds\\.trap\\.\\k<inference_mv22_vector_place>",
        "aero\\.bounds\\.trap\\.\\k<inference_mv22_vector_place>:\\r?\\n  call void @llvm\\.trap\\(\\)\\r?\\n  unreachable\\r?\\naero\\.bounds\\.safe\\.\\k<inference_mv22_vector_place>:",
        "(?<inference_mv22_vector_extended>%reg[0-9]+) = sext i32 \\k<inference_mv22_vector_index> to i64",
        "(?<inference_mv22_vector_target>%ptr[0-9]+) = getelementptr inbounds \\[2 x i32\\], \\[2 x i32\\]\\* \\k<inference_mv22_vector_base>, i64 0, i64 \\k<inference_mv22_vector_extended>",
        "(?<inference_mv22_vector_value>%reg[0-9]+) = load i32, i32\\* \\k<inference_mv22_vector_target>, align 4",
        "(?<inference_mv22_product>%reg[0-9]+) = mul i32 \\k<inference_mv22_matrix_value>, \\k<inference_mv22_vector_value>",
        "(?<inference_mv22_sum>%reg[0-9]+) = add i32 \\k<inference_mv22_accumulator>, \\k<inference_mv22_product>",
        "store i32 \\k<inference_mv22_sum>, i32\\* \\k<inference_mv22_accumulator_slot>, align 4",
        "(?<inference_mv22_output_index>%reg[0-9]+) = load i32, i32\\* \\k<inference_mv22_row_slot>, align 4",
        "(?<inference_mv22_output_lower>%reg[0-9]+) = icmp sge i32 \\k<inference_mv22_output_index>, 0",
        "(?<inference_mv22_output_upper>%reg[0-9]+) = icmp slt i32 \\k<inference_mv22_output_index>, 2",
        "(?<inference_mv22_output_inbounds>%reg[0-9]+) = and i1 \\k<inference_mv22_output_lower>, \\k<inference_mv22_output_upper>",
        "br i1 \\k<inference_mv22_output_inbounds>, label %aero\\.bounds\\.safe\\.(?<inference_mv22_output_place>[0-9]+), label %aero\\.bounds\\.trap\\.\\k<inference_mv22_output_place>",
        "aero\\.bounds\\.trap\\.\\k<inference_mv22_output_place>:\\r?\\n  call void @llvm\\.trap\\(\\)\\r?\\n  unreachable\\r?\\naero\\.bounds\\.safe\\.\\k<inference_mv22_output_place>:",
        "(?<inference_mv22_output_extended>%reg[0-9]+) = sext i32 \\k<inference_mv22_output_index> to i64",
        "(?<inference_mv22_output_target>%ptr[0-9]+) = getelementptr inbounds \\[2 x i32\\], \\[2 x i32\\]\\* \\k<inference_mv22_output_base>, i64 0, i64 \\k<inference_mv22_output_extended>",
        "(?<inference_mv22_output_value>%reg[0-9]+) = load i32, i32\\* \\k<inference_mv22_accumulator_slot>, align 4",
        "store i32 \\k<inference_mv22_output_value>, i32\\* \\k<inference_mv22_output_target>, align 4",
        "(?<inference_guard_lower>%reg[0-9]+) = icmp sge i32 (?<inference_guard_index>%reg[0-9]+), 0",
        "(?<inference_guard_upper>%reg[0-9]+) = icmp slt i32 \\k<inference_guard_index>, (?<inference_guard_bound>[0-9]+)",
        "(?<inference_guard_inbounds>%reg[0-9]+) = and i1 \\k<inference_guard_lower>, \\k<inference_guard_upper>",
        "br i1 \\k<inference_guard_inbounds>, label %aero\\.bounds\\.safe\\.(?<inference_guard_place>[0-9]+), label %aero\\.bounds\\.trap\\.\\k<inference_guard_place>",
        "aero\\.bounds\\.trap\\.\\k<inference_guard_place>:\\r?\\n  call void @llvm\\.trap\\(\\)\\r?\\n  unreachable\\r?\\naero\\.bounds\\.safe\\.\\k<inference_guard_place>:",
        "(?<inference_guard_extended>%reg[0-9]+) = sext i32 \\k<inference_guard_index> to i64",
        "getelementptr inbounds (?<inference_guard_aggregate>\\[\\k<inference_guard_bound> x i32\\]), \\k<inference_guard_aggregate>\\* %ptr[0-9]+, i64 0, i64 \\k<inference_guard_extended>",
        "ret [8 x i32]",
        "double|fptosi|fptoui|sitofp|uitofp| nsw | nuw |icmp u(gt|ge|lt|le) i32|udiv i32|urem i32|lshr i32| x i8]|<[^>]* x [^>]*>|\\[[0-9]+ x \\[|extract(element|value)|insert(element|value)|shufflevector|aero_quant|quantization",
    ];
