...
const REPLAY_EXCLUSIONS: [&str; 132] = [
...
fn fixture_schema() -> Value {
    ...
    let replay = closed_object_schema(vec![
        ...
        (
            "excluded_paths",
            array_schema(string_schema(), 132, 132, true),
        ),
        (
            "fresh_observations",
            closed_object_schema(vec![
                ("records", array_schema(fresh_observation, 0, 132, true)),
...
fn fixture_reproduce_text() -> String {
    [
        ...
        "The raw pinned-tool version, Aero LLVM-build, and public-run diagnostic streams are traceability-only replay exclusions",
...
fn validate_fresh_observations(
    ...
    if records.len() != REPLAY_EXCLUSIONS.len() {
        return Err(format!(
            "fresh observations must contain exactly {} closed trace leaves",
            REPLAY_EXCLUSIONS.len()
        ));
    }
...
fn replay_projection_excludes_only_the_closed_traceability_observations() {
    ...
    for (label, pointer, replacement) in [
        ...
        (
            "tool path",
            "/platforms/0/toolchain/tools/opt/path",
            json!("/different/opt"),
        ),
        (
            "tool version exit",
            "/platforms/0/toolchain/tools/opt/version/exit_code",
            json!(1),
        ),
        (
            "parsed tool version",
            "/platforms/0/toolchain/tools/opt/version/parsed/version",
            json!("22.1.9"),
        ),
        (
