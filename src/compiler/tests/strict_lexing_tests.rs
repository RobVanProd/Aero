...
#[test]
fn doc_rejects_lexical_error_in_direct_module_without_writing_markdown() {
    let workspace = TestWorkspace::new("strict_lex_doc_module");
    let source_path = workspace.path("main.aero");
    let module_path = workspace.path("broken.aero");
    let markdown_path = workspace.path("module-doc.md");
    fs::write(&source_path, "mod broken;").expect("write doc module root");
    fs::write(&module_path, "let value = 1@;").expect("write invalid doc module");
    ...
    assert_lex_error(&output, "broken.aero", 14, "Unexpected character '@'");
    assert!(
        !markdown_path.exists(),
        "failed direct-module doc wrote Markdown"
    );
}

#[test]
fn doc_validates_direct_module_without_appending_its_content() {
    let workspace = TestWorkspace::new("strict_lex_doc_module_valid");
    let source_path = workspace.path("main.aero");
    let module_path = workspace.path("healthy.aero");
    let markdown_path = workspace.path("module-doc.md");
    fs::write(&source_path, "mod healthy;\n\nfn root() {}\n").expect("write valid doc root");
    fs::write(&module_path, "fn imported() {}\n").expect("write valid doc module");
    ...
    assert!(
        output.status.success(),
        "valid direct-module doc failed: {diagnostics}"
    );
    let markdown = fs::read_to_string(&markdown_path).expect("read generated Markdown");
    let expected = format!(
        "# Aero API Documentation\n\nGenerated by `aero doc` for `{}`.\n\n## Modules\n\n- `mod healthy`\n\n## Functions\n\n### `fn root()`\n\n## Structs\n\n_No struct declarations found._\n\n## Enums\n\n_No enum declarations found._\n\n## Traits\n\n_No trait declarations found._\n",
        source_path.display()
    );
    assert_eq!(markdown, expected, "direct module changed root-only docs");
    assert!(
        !markdown.contains("imported"),
        "direct-module declarations leaked into root docs"
    );
}
