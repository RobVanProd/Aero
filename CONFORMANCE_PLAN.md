...
profiles. CAP-024 adds immutable accepted-head evidence only. CAP-015 remains the
accepted M1-001 representative-integration checkpoint.
...
Accepted CAP-024 records the immutable accepted-head correctness, reproducibility,
observable-behavior, and artifact-footprint evidence-only checkpoint under the closed
recorded boundary.
...
Accepted CAP-023 historically added one zero-production source-embedded fixed-shape
...
CAP-023 add product evidence only. CAP-024 records immutable accepted-head evidence
only, while CAP-015 only enriches the M1-001 `END_TO_END` row. General-purpose
