...
| R-004 | Ownership claims exceed enforcement and permit dangling/aliased/moved values | HIGH | CRITICAL | Bounded whole-place enum ownership, exact acyclic joins, and independent checked-CFG consumption proof exist through accepted CORE-075; local candidate CORE-076 unifies typed Match results over recursive CopyData and the existing owned-enum class without adding a CopyData move. Lifetime provenance is absent and mutable references remain classified as `Copy` outside the admitted boundaries. | Preserve the finite classifiers and compile-fail suite; separately freeze loop fixed points, projections/partial moves, borrow provenance, drop/destructors, lifetimes, and general CFG ownership | OPEN — exact local controls are additive, but no general ownership or memory-safety claim follows |
...
| R-006 | CLI and library compile through divergent module instances | HIGH | HIGH | Accepted `CORE-011` at `a711dd5` centralizes direct-module collection; accepted CORE-020 closes ignored nondefaults; accepted CORE-070 at `365c28a3e4fdd306ec4c1a4837545ddbe3dac6a3` adds public file-aware library compilation over the same direct collector and checked library frontend, with byte-exact module-free parity and exact-head public gates | Preserve the shared collector, source-only/default boundaries, and file-aware contract; separately design canonical library/thin-CLI orchestration, imports/namespaces, recursion, cache identity, external verification, and real option semantics | PARTIALLY CONTROLLED — direct-module, ignored-option, and file-aware library boundaries are publicly closed; CLI/tool orchestration and full module semantics remain open |
...
Local candidate `CORE-076` adds an R-005 control and narrows R-011's Match-result surface
...
private LLVM types. The 194-library/200-binary surface, complete local gates, direct
modules, artifact hygiene, deterministic LLVM, and tracked two-module exit-223 specimen
pass locally; immutable publication and exact public/native acceptance remain pending.
