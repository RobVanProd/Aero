...
| R-007 | Backend labels are mistaken for device execution | HIGH | HIGH | `AUDIT-024` at `9ddc571` proves ROCm `run` can return zero after unchecked temporary object generation with no link/launch; CUDA is unavailable; `gpu` is a heuristic that can choose CPU; current CLI/docs overstate scalar graph/quant transforms and an enabled Aero GGUF example | Fail object-only `run` closed, require object postconditions and explicit target choice, reclassify current claims, then require separate hardware execution/correctness gates | OPEN — `CORE-018` preregistered; no Aero device evidence |
