...
| R-007 | Backend labels are mistaken for device execution | HIGH | HIGH | `AUDIT-024` at `9ddc571` proved the false success; triple-approved tests-only `427fb4c` reproduced the exact public red boundary, and the local `CORE-018` candidate now passes CLI 10/10, claims 7/7, and the full gate with fail-closed ROCm/CUDA, explicit targets, non-device telemetry, and the Aero GGUF route disabled | Complete exact-review/public-green acceptance for the selected false-success boundary, then require separate hardware execution/correctness gates for any device claim | OPEN — selected correction is an unaccepted implementation candidate; no Aero device evidence |
...
The complete gate is green; acceptance still requires exact three-review approval
and public green CI. Even after that selected false-success correction, R-007 remains
open for real Aero accelerator execution and correctness evidence.
