...
| R-004 | Ownership claims exceed enforcement and permit dangling/aliased/moved values | HIGH | CRITICAL | Bounded whole-place enum ownership, exact acyclic joins/reinitialization, and independent checked-CFG consumption proof exist through accepted CORE-076; local CORE-077 adds only exact balanced restoration of a direct mutable enum owner on every reachable loop edge. Lifetime provenance is absent and mutable references remain classified as `Copy` outside the admitted boundaries. | Preserve the finite classifiers and compile-fail suite; separately freeze loop-carried moved state, projections/partial moves, borrow provenance, drop/destructors, lifetimes, and general CFG ownership | OPEN — exact local controls are additive, but no general ownership or memory-safety claim follows |
...
Local candidate `CORE-077` adds a bounded R-004/R-005 control without closing either
...
exit repairs. The 195-library/201-binary complete local gates, direct modules, CLI
artifact hygiene, deterministic LLVM, and tracked exit-227 specimen pass locally;
immutable publication and exact public/native acceptance remain pending. Loop-carried
