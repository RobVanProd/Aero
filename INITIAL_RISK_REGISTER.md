...
| R-003 | Unsupported expressions are accepted with invented integer/zero semantics | HIGH | CRITICAL | Semantic and IR fallback arms for aggregates, fields, matches, closures, methods, borrows; `%` passes semantics then panics because IR has no remainder operation | `CORE-005` preregisters fail-closed modulo; retain separate boundaries for invented-zero and unsupported comparison families; eventually make IR fallible | OPEN — modulo rejection preregistered |
...
| R-005 | Invalid programs pass semantics then panic, miscompile, or produce invalid LLVM | HIGH | CRITICAL | Fatal parser propagation is controlled at `6ce85922`; numeric scalar/callable IR scope restoration and semantic compatibility-scope isolation are controlled at `bc9a148`; unreachable statements can still follow terminators, the original AST crosses the "typed" boundary, IR is infallible, backend fallbacks remain, and no active verifier exists | Typed representation and verifier gates; fail-closed unsupported expressions; isolate unreachable-CFG cleanup | OPEN — parser/local epilogues and tested lexical scope paths controlled |
