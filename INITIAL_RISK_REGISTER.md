...
| R-006 | CLI and library compile through divergent module instances | HIGH | HIGH | Both `lib.rs` and `main.rs` declare overlapping compiler modules; `AUDIT-017` confirms repeated direct-module collection, source-only library `ModDecl` acceptance, build missing-module artifact publication, and root-only cache identity | Execute bounded `CORE-011` shared fail-closed source collection, then separately design canonical library/thin-CLI orchestration and `CompilerOptions` | OPEN — `CORE-011` preregistered for the direct-module boundary; full pipeline convergence remains separate |
...
The current implementation task is preregistered `CORE-011`, selected after
...
namespace, visibility, import, or recursive path semantics.
