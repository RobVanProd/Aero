...
| R-013 | User-facing commands report success without promised behavior | HIGH | HIGH | Accepted public `CORE-013` at `a78dd00` controls typed `0/1/2` CLI-owned statuses; triple-reviewed tests-only `6728a39` publicly proves the semantic-only `aero test` execution/pass claim with exact compiler/nightly 9/2 failures while all four CodeQL checks pass; the bounded local wording candidate is 11/11 with exact full-gate success | Preserve typed statuses and the analysis-only presentation; require exact review/public-green acceptance; delegated exits, atomic rollback, executable-test design, command maturity, and helper architecture remain separate | PARTIALLY CONTROLLED — statuses closed; selected presentation control has public red and local full-gate proof, but acceptance remains pending |
