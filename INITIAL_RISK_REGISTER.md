...
| R-006 | CLI and library compile through divergent module instances | HIGH | HIGH | Accepted `CORE-011` at `a711dd5` centralizes direct-module collection; accepted CORE-020 closes ignored nondefaults; local CORE-070 candidate adds public file-aware library compilation over the same direct collector and checked library frontend, with byte-exact module-free parity and complete local gates | Preserve the shared collector, source-only/default boundaries, and file-aware contract; separately design canonical library/thin-CLI orchestration, imports/namespaces, recursion, cache identity, external verification, and real option semantics | PARTIALLY CONTROLLED — direct-module, ignored-option, and candidate file-aware library boundaries are closed locally; CLI/tool orchestration and full module semantics remain open |
...
Local candidate `CORE-070` partially reduces R-006 by adding an exact root-file library
route over the existing direct-module collector and the same checked frontend/codegen
sequence as `compile_program`. Its success/failure matrix and full local gates pass,
but public acceptance is pending and the CLI, other tools, imports/namespaces,
