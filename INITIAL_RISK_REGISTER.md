...
| R-012 | Dormant, duplicated, or ignored tests create false coverage confidence | HIGH | HIGH | `AUDIT-023` at clean `8869eca` reproduces 38 ignored / 36 pass / 2 fail and classifies 4 lexer, 20 parser, and 14 semantic tests; only 22 strict syntax-retention tests are safe activation candidates, while 14 semantic and 2 generic-impl tests remain quarantined; Cargo also compiles overlapping library/binary test names and 299 dormant tests remain | Activate only the exact strict 22-test syntax subset; bind exact token/retained AST shape and the 16-test quarantine; report target entries, overlap, distinct evidence, and gates separately | OPEN — `CORE-017` preregistration candidate; no activation or semantic capability change yet |
