...
| R-006 | CLI and library compile through divergent module instances | HIGH | HIGH | Accepted `CORE-011` at `a711dd5` centralizes direct-module collection across both compiled trees; `AUDIT-026` at public-green preregistration `2c61ff9` inventories 62 default-only callers and proves public nondefault `CompilerOptions` are silently ignored across checked compilation | Preserve the shared collector; `CORE-020` must reject every nondefault option before lexing without defining option/CLI/backend behavior; separately design canonical library/thin-CLI orchestration | PARTIALLY CONTROLLED — direct-module source-set boundary is closed; ignored-option containment is selected but not implemented; duplicated orchestration and full pipeline convergence remain open |
...
| R-013 | User-facing commands report success without promised behavior | HIGH | HIGH | Accepted public `CORE-013` at `a78dd00` controls typed `0/1/2` CLI-owned statuses; tests-only `6728a39` proves the semantic-only `aero test` execution/pass claim; implementation `2fe580d`, closure `63b6629`, and final sync `25dec51` pass their gates and public checks with analysis-only/no-execution presentation | Preserve typed statuses and the accepted analysis-only presentation; delegated exits, atomic rollback, executable-test design, command maturity, and helper architecture remain separate | PARTIALLY CONTROLLED — selected `aero test` presentation accepted; remaining command-behavior risks stay open and `AUDIT-026` re-ranks them read-only |
