...
| R-003 | Unsupported expressions are accepted with invented integer/zero semantics | HIGH | CRITICAL | Semantic and IR fallback arms for aggregates, closures, methods, and borrows remain; `%` is fail-closed at `302211e`; tuple values are controlled at reviewed `cbbe049`; named fields at reviewed `4e10d479`; Match at reviewed `b74d91a`; StructLiteral has a complete-gate fail-closed candidate at `3410f1f`, with exact-candidate review corrections in progress | Finish independent StructLiteral review; retain separate Enum/ownership/method/comparison/division boundaries; make IR fallible and verified | OPEN — modulo, tuple values, named fields, and trusted Match controlled; StructLiteral candidate awaiting review; other families remain |
