...
| R-013 | User-facing commands report success without promised behavior | HIGH | HIGH | Accepted `CORE-013` controls typed CLI statuses; accepted `CORE-019` controls analysis-only `aero test` wording; public-green `AUDIT-027` basis `aa3e7a8` selects `CORE-021`; exact reviewed tests-only `0873f65` publicly reproduces nonzero delegated false success while CodeQL remains green; the one-condition production candidate passes focused/full local gates | Preserve typed statuses and accepted analysis-only wording; make delegated success wording conditional on zero; keep init containment/rollback, executable-test design, command maturity, and helper architecture separate | PARTIALLY CONTROLLED — prior slices accepted; delegated nonzero false-success implementation is not accepted until exact-review/public/record closure; other command boundaries remain open |
...
tests, while CodeQL `30839264268` and aggregate `91772180985` pass. R-013 remains
partially controlled until implementation full-gate, exact-review, public acceptance,
and closure pass; all unselected risks remain open.
