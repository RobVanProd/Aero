...
| R-002 | Calls, annotations, and returns violate declared type contracts | HIGH | CRITICAL | Monomorphic numeric/void function calls and returns are controlled at `8d5d8e7`; initialized exact numeric annotations are controlled at `bc9a148`; `AUDIT-021` at `1535ce2` reproduces five nonnumeric/fixed-array annotation mismatches that pass check/build and publish LLVM because semantics and checked IR discard those annotations | Outside active generic semantic scopes, enforce exact bool/canonical-String/nonempty-flat-fixed-numeric-array equality and numeric-array element/count/index inference; preserve green controls for lowercase string, custom/contextual/structural annotations, empty/nonnumeric arrays, and generic-scope annotation/array behavior | PARTIALLY CONTROLLED — `CORE-015` preregistered for four active false successes; excluded and generic-scope gaps remain open |
...
| R-011 | Aggregate and array lowering changes types or crashes | HIGH | HIGH | `AUDIT-021` confirms semantics infers only the first array element and ignores index type; mixed `[1, 2.5]` and float indexing now fail without artifacts only in verifier/checked-IR phases, while wrong explicit array element/count annotations still publish LLVM | Move exact homogeneity, count, and integer-index controls before IR in `CORE-015`; typed aggregate IR/bounds/layout remain separately required before execution claims | OPEN — selected phase-order subset is coupled to `CORE-015` |
| R-012 | Dormant, duplicated, or ignored tests create false coverage confidence | HIGH | HIGH | At `1535ce2`, Cargo lists 139 library and 148 binary tests with 105 shared names; `--tests -- --list` yields 557 target entries/437 distinct displayed names; 38 phase-5 tests remain ignored and the initial audit found 299 dormant source tests | Classify backlog; restore valid tests deliberately; report target entries, displayed-name overlap, distinct behavior evidence, and gates separately | OPEN |
