...
| R-013 | User-facing commands report success without promised behavior | HIGH | HIGH | Accepted public `CORE-013` at `a78dd00` controls the typed `0/1/2` CLI-owned status boundary; `AUDIT-025` at `d0bd54e` confirms that semantic-only `aero test` still says `run`, `Running`, and `passed` without checked IR, codegen, or execution | Preserve typed statuses; make semantic-only test presentation explicit; delegated exits, atomic rollback, executable-test design, command maturity, and helper architecture remain separate | PARTIALLY CONTROLLED — CLI-owned statuses closed; active test-execution/pass claim selected by `CORE-019` |
