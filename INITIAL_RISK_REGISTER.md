...
| R-002 | Calls, annotations, and returns violate declared type contracts | HIGH | CRITICAL | Monomorphic numeric/void function calls and returns are controlled at `8d5d8e7`; numeric `let` annotations and boolean/generic/composite function contracts remain unenforced or uncertified | Preregister the smallest numeric annotation slice; retain separate typed boundaries for booleans and richer types | PARTIALLY CONTROLLED — numeric function boundary closed |
