...
| R-005 | Invalid programs pass semantics then panic, miscompile, or produce invalid LLVM | HIGH | CRITICAL | Fatal parser propagation is controlled at `6ce85922`; review of `5fa5a5e` found a valid nested shadow loading the inner scalar after scope exit; review of `b6b0eba` found a then-arm-only semantic name reaching IR and panicking; unreachable statements can still follow terminators, the original AST crosses the "typed" boundary, IR is infallible, backend fallbacks remain, and no active verifier exists | `CORE-004` IR and semantic compatibility binding-snapshot restoration; typed representation and verifier gates; isolate unreachable-CFG cleanup | OPEN — parser/local epilogues controlled, semantic scope correction preregistered |
