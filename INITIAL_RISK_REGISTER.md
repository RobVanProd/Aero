...
| R-008 | Public 1.0/formal/safety messaging outruns evidence | HIGH | HIGH | `AUDIT-022` at `c612f3b` reproduces the mismatch; reviewed public red `4b94dbd` binds exactly 2 preservation passes / 5 claim failures; exact three-review-approved implementation `cc984d0` derives CLI presentation from package metadata, classifies three cases/four repetitions as regression evidence, visibly bounds generic/ownership/safety documents, and passes focused/full local gates plus all eight public checks | Preserve manifest-derived CLI implementation version; distinguish the v1.0.0 language design target; keep conformance compatibility schema/counts unchanged; retain visible design/history qualifications and unsupported safety/type boundaries | CANDIDATE CONTROLLED — implementation evidence is public and green; R-008 remains open only until record-only acceptance closure is exactly reviewed and publicly green; no version/release change authorized |
...
is inferred. Exact three-review-approved implementation `cc984d0` is now public and
all eight checks pass; only record-only acceptance closure remains before changing
the R-008 status to controlled.
