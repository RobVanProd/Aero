...
| R-003 | Unsupported expressions are accepted with invented integer/zero semantics | HIGH | CRITICAL | Semantic and IR fallback arms for aggregates, fields, matches, closures, methods, borrows remain; `%` is fail-closed at `302211e`; `CORE-006` preregisters recursive rejection of tuple literals/projections after `(7, 9).0` was proven to emit zero at `704b3328` | Retain separate boundaries for each fabricated-zero family and unsupported comparisons; close tuples tests-first; eventually make IR fallible | OPEN — modulo controlled; tuple boundary preregistered; other fabricated-zero families remain |
