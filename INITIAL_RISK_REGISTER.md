...
| R-013 | User-facing commands report success without promised behavior | HIGH | HIGH | Accepted `CORE-013` controls typed CLI statuses; accepted `CORE-019` controls analysis-only `aero test` wording; public-green `AUDIT-027` basis `aa3e7a8` selects `CORE-021`; tests-only `0873f65` publicly reproduces nonzero delegated false success; exact reviewed/public-green implementation `a4327be` conditions the success line on zero | Preserve typed statuses, accepted analysis-only wording, and delegated status/output/cleanup controls; keep init containment/rollback, executable-test design, command maturity, and helper architecture separate | PARTIALLY CONTROLLED — delegated nonzero false-success wording is accepted at `a4327be`; other command boundaries remain open |
