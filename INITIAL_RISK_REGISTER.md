...
| R-014 | Quick Start and flagship examples fail new-user workflows | HIGH | MEDIUM | `AUDIT-020` at `18526ff` reproduces root `cargo build --release` exit 101, the wrong root binary path, and the unsupported `aeronum`/`aeronn` flagship; public red checkpoint `fc77e99` reproduces the three frozen gaps; the documentation/workflow candidate passes the 5/5 focused target and exact full local gate | Run the exact minimal generated-project path in tests/CI; correct manifest/binary paths; remove unsupported syntax from the flagship | IMPLEMENTATION CANDIDATE — local control green; exact review and public CI acceptance pending |
