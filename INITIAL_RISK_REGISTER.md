...
| R-005 | Invalid programs pass semantics then panic or produce invalid LLVM | HIGH | CRITICAL | Original AST crosses "typed" boundary; infallible IR; backend type fallbacks; no active verifier | Typed representation and verifier gates before backend/object output | OPEN |
...
| R-013 | User-facing commands report success without promised behavior | HIGH | HIGH | `test` does not execute; some CLI branches print errors then return normally | Exit-status contract tests and command maturity classification | OPEN |
