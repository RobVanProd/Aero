...
| R-003 | Unsupported expressions are accepted with invented integer/zero semantics | HIGH | CRITICAL | Semantic and IR fallback arms for aggregates, fields, matches, closures, methods, borrows remain; `%` is fail-closed at `302211e`; tuple values are controlled at reviewed `cbbe049`; named fields fail closed in candidate `75dbfba` after real and undeclared receivers were proven to emit zero at `52d3415` | Retain separate boundaries for each remaining fabricated-zero family and unsupported comparisons; independently review named-field closure; eventually make IR fallible | OPEN — modulo and tuple values controlled; named-field candidate controlled; other families remain |
