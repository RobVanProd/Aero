...
| R-003 | Unsupported expressions are accepted with invented integer/zero semantics | HIGH | CRITICAL | Semantic and IR fallback arms for aggregates, fields, matches, closures, methods, borrows remain; `%` is fail-closed at `302211e`; tuple literals/projections fail closed recursively at `1fa67a2` after `(7, 9).0` was proven to emit zero | Retain separate boundaries for each remaining fabricated-zero family and unsupported comparisons; eventually make IR fallible | OPEN — modulo and tuple candidate controlled; other fabricated-zero families remain |
