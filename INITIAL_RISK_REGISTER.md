...
| R-003 | Unsupported expressions are accepted with invented integer/zero semantics | HIGH | CRITICAL | Semantic and IR fallback arms for aggregates, matches, closures, methods, and borrows remain; `%` is fail-closed at `302211e`; tuple values are controlled at reviewed `cbbe049`; named fields are controlled at reviewed `4e10d479`; `AUDIT-013` proves Match zero/drop behavior and selects preregistered `CORE-008` | Retain separate boundaries for each remaining fabricated-zero family and unsupported comparisons; preserve Match red evidence and complete its independent fail-closed gate; eventually make IR fallible | OPEN — modulo, tuple values, and named fields controlled; Match preregistered; other families remain |
