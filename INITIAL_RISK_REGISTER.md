...
| R-008 | Public 1.0/formal/safety messaging outruns evidence | HIGH | HIGH | `AUDIT-022` at `c612f3b` reproduces package `0.3.0` versus CLI/banner `1.0.0`; conformance has three example cases and four deterministic repetition checks; current-facing docs still present design-target generics, ownership, memory safety, and formal/mechanized proof as implemented | Make Cargo package version the CLI implementation-version source; distinguish the v1.0.0 language design target; classify conformance as regression evidence and unsupported safety/type surfaces as design-only | OPEN — `CORE-016` selected and preregistered; no version/release change authorized |
