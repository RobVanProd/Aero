...
| R-003 | Unsupported expressions are accepted with invented integer/zero semantics | HIGH | CRITICAL | Semantic and IR fallback arms for aggregates, fields, matches, closures, methods, borrows remain; `%` is fail-closed at `302211e`; tuple values are controlled at reviewed `cbbe049`; `CORE-007` preregisters recursive field-value rejection after real and undeclared receivers were proven to emit zero at `52d3415` | Retain separate boundaries for each remaining fabricated-zero family and unsupported comparisons; close named fields tests-first; eventually make IR fallible | OPEN — modulo and tuple values controlled; field boundary preregistered; other families remain |
