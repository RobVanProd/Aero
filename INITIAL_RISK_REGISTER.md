...
| R-008 | Public 1.0/formal/safety messaging outruns evidence | HIGH | HIGH | `AUDIT-022` at `c612f3b` reproduces package `0.3.0` versus CLI/banner `1.0.0`; conformance has three example cases and four deterministic repetition checks; current-facing docs still present design-target generics, ownership, memory safety, and formal/mechanized proof as implemented; public green preregistration `1575914` plus the local 2-pass/5-fail focused red target bind those gaps | Make Cargo package version the CLI implementation-version source; distinguish the v1.0.0 language design target; classify conformance as regression evidence and unsupported safety/type surfaces as design-only | OPEN — `CORE-016` tests-only red checkpoint awaits exact review/publication; no version/release change authorized |
