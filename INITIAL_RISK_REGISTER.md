...
| R-013 | User-facing commands report success without promised behavior | HIGH | HIGH | Accepted public `CORE-013` at `a78dd00` controls typed `0/1/2` CLI-owned statuses; tests-only `6728a39` publicly proves the semantic-only `aero test` execution/pass claim; exact three-review-approved implementation `2fe580d` passes focused 11/11, the full gate, and all eight public checks with analysis-only/no-execution presentation | Preserve typed statuses and the accepted analysis-only presentation; delegated exits, atomic rollback, executable-test design, command maturity, and helper architecture remain separate | PARTIALLY CONTROLLED — CLI-owned statuses and the selected `aero test` presentation boundary are accepted; remaining command-behavior risks stay open |
