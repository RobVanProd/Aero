...
| R-006 | CLI and library compile through divergent module instances | HIGH | HIGH | Accepted `CORE-011` centralizes direct-module collection; accepted CORE-020 closes ignored nondefaults; accepted CORE-070 adds file-aware library compilation. Locally green CORE-081 has an exact 35-overlap red and a one-library-graph implementation with unit/integration/all-features/static/root evidence; public gates remain pending. | Preserve one library-owned phase graph, shared collector, source-only/default boundaries, and file-aware contract; separately converge remaining tool orchestration, imports/namespaces, recursion, cache policy, external verification, and real option semantics | PARTIALLY CONTROLLED — public direct-module/options/file-aware boundaries are closed and local module-instance duplication is removed; public CORE-081 acceptance and broader tool/module semantics remain open |
...
| R-012 | Dormant, duplicated, or ignored tests create false coverage confidence | HIGH | HIGH | `AUDIT-023` classified 38 ignored tests; accepted `CORE-017` runs 22 strict retention tests and keeps 16 explicit quarantines. Locally green CORE-081 moves six compatibility/optimization tests to the library owner and removes duplicate binary compiler-module execution without deleting a distinct test; 207 library, 32 binary, and exact root gates pass, with public evidence pending. | Preserve exact strict token/retained-AST assertions and the 16-test quarantine; report target entries, overlap, distinct evidence, and gates separately; mechanically enforce one compiler graph | PARTIALLY CONTROLLED — selected Phase 5 classification is public; local graph duplication is removed, while CORE-081 public acceptance and 299 dormant tests remain open |
...
| R-018 | The cumulative draft integration PR grows beyond a reviewable and controllable checkpoint | HIGH | CRITICAL | PR #4 is already 281 commits across 224 files and is operating as an integration program rather than a normal review unit | Freeze a separately authorized controlled-checkpoint strategy after CORE-081: immutable accepted head, review/merge criteria, evidence handoff, rollback/recovery, and a bounded successor PR before another language slice | OPEN — urgent; no merge, history rewrite, release, or new stacking is authorized by this classification |
...
Active candidate `CORE-081` directly reduces R-006 and R-012. The binary no longer
...
and exact root evidence is green; immutable public gates remain pending. R-006 stays partially controlled
