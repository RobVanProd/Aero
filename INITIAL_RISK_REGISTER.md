...
| R-012 | Dormant, duplicated, or ignored tests create false coverage confidence | HIGH | HIGH | `AUDIT-023` at clean `8869eca` classified 38 ignored tests; public-green preregistration `2c61535` selected a strict 22/16 split; exact three-review-approved implementation `8be8c21` has exactly 4 strict lexer + 18 strict parser-retention passes, 16 explicit quarantines, 38 listed entries, exact full-gate success, all eight public checks green, and no production change; Cargo overlap and 299 dormant tests remain | Preserve exact strict token/retained-AST assertions and the 16-test quarantine; report target entries, overlap, distinct evidence, and gates separately | CANDIDATE PARTIALLY CONTROLLED — implementation evidence is public and green; R-012 remains open only until record-only acceptance closure is exactly reviewed and publicly green; no semantic capability change |
...
remains candidate-partially-controlled only until the record-only acceptance closure;
the 299 dormant tests and library/binary Cargo overlap remain open residual evidence
risks, and no semantic capability is inferred.
