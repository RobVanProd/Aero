...
| R-003 | Unsupported expressions are accepted with invented integer/zero semantics | HIGH | CRITICAL | Semantic and IR fallback arms for aggregates, closures, methods, and borrows remain; `%` is fail-closed at `302211e`; tuple values are controlled at reviewed `cbbe049`; named fields at reviewed `4e10d479`; Match at reviewed `b74d91a`; StructLiteral is fail-closed at reviewed `daa024d` | Execute `CORE-010` generic checked-IR rejection for every unadmitted fallback while retaining separate Enum/ownership/method/comparison/division implementation semantics | PARTIALLY CONTROLLED — modulo, tuple values, named fields, trusted Match, and StructLiteral controlled; other families remain open |
...
| R-005 | Invalid programs pass semantics then panic, miscompile, or produce invalid LLVM | HIGH | CRITICAL | Fatal parser propagation is controlled at `6ce85922`; numeric scalar/callable scope isolation is controlled at `bc9a148`; modulo is fail-closed at `302211e`; `AUDIT-016` reproduced IR unwinds, invalid Boolean LLVM, infallible APIs, silent codegen fallback, and absent verification | Execute preregistered `CORE-010` checked logical IR plus internal/external verifier gates; retain separate CFG cleanup | OPEN — parser/local epilogues, tested lexical scopes, modulo, and several unsupported forms controlled; typed IR/verification pending |
