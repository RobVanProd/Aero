...
| R-013 | User-facing commands report success without promised behavior | HIGH | HIGH | Accepted `CORE-013` controls typed CLI statuses; accepted `CORE-019` controls analysis-only `aero test` wording; accepted `CORE-021` at closure `b99e445` controls delegated nonzero success wording; public-green `AUDIT-028` basis `399e04f` selects dangling-entry init containment under `CORE-022` | Preserve accepted status/wording controls; refuse occupied init destinations before writes; keep general rollback/atomicity/race freedom, executable-test design, command maturity, and helper architecture separate | PARTIALLY CONTROLLED — prior status/presentation slices accepted; dangling-entry preflight selected but not implemented; other command boundaries remain open |
...
partially controlled until `CORE-022` tests-first, implementation, review, public, and
record closure pass; all unselected residuals remain open.
