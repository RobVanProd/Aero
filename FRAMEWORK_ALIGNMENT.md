...
Local candidate `CORE-085` composes those two bounded directions without widening
...
loan is live fail before trusted LLVM. Focused tests, corruption controls, and the local
CLI system specimen pass with expected native exit 85 pinned in public workflows; the
exact repository gate passes 213 library tests, 32 binary tests, every integration
target, and doc tests. Candidate-head and public acceptance remain pending. This is
local candidate evidence, not public acceptance or a general borrowing, lifetime/NLL,
drop, stable ABI, FFI, or memory-safety claim.
...
| Ownership-based safety | Shallow move checks remain partial. CORE-048/053 through accepted CORE-066 establish bounded immutable/mutable whole-place ownership, internal reference transport, recursive finite CopyData composition, direct CopyData owner reassignment, owned enum transport/replacement, exact acyclic conditional joins, independent enum-owner CFG consumption proof, and fresh per-iteration enum owners. Accepted CORE-073–077 add acyclic reinitialization, owned Match results, and balanced loop restoration. Accepted CORE-079 adds convergent `Owned`/`Moved`/`MaybeMoved` direct-enum loop headers/exits under shared source/admission classification and independent verifier proof. Accepted CORE-083 composes exclusive whole-place mutable reference transport with admitted enums under pinned exit-83 proof. Accepted CORE-084 adds only repeated exhaustive Match reads through multiple immutable aliases to initialized immutable direct enum owners under pinned exit-84 proof. Local candidate CORE-085 composes multiple immutable observation loans with mutable direct enum owners, exact lexical ends, and CFG loan-state proof; public acceptance is pending. Mutable projections, reference results, escaping provenance, free enum dereference/transport, enum aggregate storage, stable reference ABI, general non-enum CFG ownership, NLL, drop, lifetime inference, and memory-safety claims remain absent; 16 broader semantic/lossy-shape Phase 5 tests remain quarantined. | Complete CORE-085's exact repository/native/public gates, then freeze another hard module, runtime, ownership, or execution slice without broadening the accepted reference boundary |
| Structs, arrays, enums, traits, and Match | CORE-043 through CORE-047 accept bounded all-Copy scalar/named-struct construction, projection, arrays, transport, and finite acyclic graphs. CORE-049 through CORE-052 accept unit/unary-scalar enums, exhaustive bound Match, and owned internal transport. CORE-058 through CORE-061 add flat tuples, whole-place references, and direct CopyData owner replacement. Accepted CORE-062 removes the executable CopyData topology whitelist. Accepted CORE-063 carries that recursive class through unary owned-enum payloads and exact bound Match under a pinned native exit-113 gate. Accepted CORE-064/065 add exact enum replacement and acyclic joins. Accepted CORE-069 generalizes positional variants to two or more recursive CopyData fields. Accepted CORE-073–077 add reinitialization, typed owned Match results, and balanced loop-owner restoration; accepted CORE-079 changes loop dataflow only, not enum topology. Accepted CORE-083 adds only mutable whole-place reference replacement for the admitted enum class. Accepted CORE-084 adds only immutable-reference exhaustive Match observation over that same class. Local candidate CORE-085 adds only immutable observation loans from mutable direct owners of the same admitted class; public acceptance is pending. Generic/named-field enums, Option/Result Match, wildcard/guard/nested destructuring, enum fields/arrays, free enum dereference or transport through references, projection/partial mutation, unit/unary tuples, unsupported/cyclic structs, dynamic arrays, traits, and stable aggregate/reference ABI remain open. | Complete the bounded CORE-085 proof, then separately freeze the next generic/module/runtime/ownership class without inferring broader enum/reference or ABI support |
