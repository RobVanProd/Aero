...
execution, public layout/ABI, safety, stability, or performance. The next action is
`CAP-015-READINESS`, not automatic implementation: freeze an exact ASCII grammar and
result for a bounded embedded-data `[char; N]` parser/interpreter, then run the full
program as a red probe without widening `exact-i32-array-v0`. If it is already green,
install the representative gate and rerank; if exactly one established at-most-two-
phase class blocks it, close that class; if profile fusion, semantic invention, or a
wider architecture is required, stop for a decision. Positive module/import
resolution follows only under a separately frozen namespace and graph contract, while
runtime file-byte acquisition remains blocked on unfrozen path, buffer, error,
ownership, and platform semantics.
