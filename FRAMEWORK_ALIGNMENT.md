...
| Ownership-based safety | Shallow move checks remain partial. Accepted CORE-048 adds non-escaping immutable scalar aliases; CORE-049 through CORE-052 add owned enum consumption and transport; CORE-053 carries immutable scalar references through exact internal calls; CORE-054 adds explicit scalar mutation; CORE-055 adds one lexical non-escaping mutable scalar alias; CORE-056 adds one direct call-scoped mutable owner loan; and CORE-057 adds exact child reborrows from that alias or the current mutable-reference parameter. Every accepted slice uses shared classification, checked identity/provenance, independent verification, typed LLVM, all-eight public checks, and pinned native execution through exit 253. Mutable-reference results, escaping provenance, stable reference ABI, general CFG ownership, NLL, drop, lifetime inference, and memory-safety claims remain absent; 16 broader semantic/lossy-shape Phase 5 tests remain quarantined | Add CFG-aware mutable/move/lifetime/destruction conformance without broadening semantics |
| Structs, arrays, enums, traits, and Match | CORE-043 through CORE-047 accept bounded all-Copy scalar/named-struct construction, projection, arrays, transport, and finite acyclic aggregate graphs. CORE-049 through CORE-052 accept unit/unary-scalar enums, exhaustive bound Match, and owned internal transport. CORE-053 through CORE-057 compose the aggregate/ADT surface with immutable and tightly bounded mutable scalar-reference behavior through pinned exit 253. CORE-058 is a local candidate for one anonymous-product layout: flat immutable arity-two-or-greater `Int`/`Float`/`Bool` tuples with Copy aliases, constant projection, scalar/tuple-only internal transport, checked identities, verification, typed literal-aggregate LLVM, and local native exit 23. Aggregate/generic/multi-field enums, tuple containers/fields/payloads/references, Option/Result Match, wildcard/guard/destructuring, direct nested/Bool arrays, cyclic aggregates, dynamic bounds, traits, and stable aggregate/reference ABI remain open | Complete public pinned evidence for the bounded tuple product, then extend another hard ownership, trait, module, or layout class without broadening semantics |
