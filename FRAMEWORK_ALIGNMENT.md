...
| Structs, enums, traits, and Match | CORE-043 provides one bounded scalar-struct value slice through shared source classification, logical checked IR, verification, LLVM, and an accepted public exit-53 native gate; ownership, ABI, recursive aggregates, enums, traits, and Match remain open | Extend aggregate composition without broadening layout/ownership semantics, then deliver an ADT/Match slice with the same proof depth |
