...
| Ownership-based safety | Shallow move checks remain partial. The unpublished CORE-048 candidate adds one real non-escaping immutable-reference subset for local `Int`/`Float`/`Bool` places through a shared classifier, checked alias-place provenance, verification, typed LLVM, and a green 159/165 local root gate. Pinned exit-127 native evidence remains pending. Mutable loans, reference ABI, NLL, drop, and general lifetime provenance remain absent; 16 broader semantic/lossy-shape Phase 5 tests remain quarantined | Accept the bounded candidate only after pinned native evidence, then add CFG-aware mutable/move/lifetime/destruction conformance without broadening semantics |
| Structs, arrays, enums, traits, and Match | CORE-043 provides an accepted bounded scalar-struct value slice. Accepted CORE-044 follows the framework's all-components-Copy rule for local aliases and exact by-value internal function transport through shared source classification, logical checked IR, verification, LLVM, and a pinned exit-63 public native gate. Accepted CORE-045 extends those exact Copy values into local fixed aggregate storage with a shared schema/count contract, distinct checked array IR, verified typed LLVM, and a pinned multi-file exit-77 public native gate. Accepted CORE-046 carries only those existing flat Copy arrays through internal by-value function boundaries using shared classification, exact checked/LLVM aggregates, all eight public checks, and pinned native exit 91. Accepted CORE-047 applies the all-components-Copy rule recursively to finite acyclic named struct graphs with scalar, named-struct, and flat fixed numeric/struct-array fields; all eight checks and its pinned exit-107 native gate pass. Non-Copy ownership, direct nested/Bool arrays, cyclic aggregates, dynamic bounds, stable ABI, enums, traits, and Match remain open | Extend non-Copy ownership or another hard class without broadening layout/ABI semantics before delivering an ADT/Match slice with the same proof depth |
