...
| Ownership-based safety | Shallow move checks remain partial. Accepted CORE-048 adds one real non-escaping immutable-reference subset for local `Int`/`Float`/`Bool` places through a shared classifier, checked alias-place provenance, verification, typed LLVM, all eight public checks, and pinned native exit 127. Accepted CORE-049 adds exact non-`Copy` move/consume behavior for owned local unit-enum values, including conservative nested Match consumption, through the same source-to-native proof depth and pinned exit 149. Accepted CORE-050 extends that transfer across exact internal parameters, calls, and returns through one recursive consumed-name classifier, direct checked SSA binding, verified LLVM flow, and pinned exit 173. Accepted CORE-051 preserves enum consumption while copying only the selected scalar payload into an arm-local binding at pinned native exit 181. Accepted CORE-052 carries the whole tagged non-`Copy` enum through exact internal parameters, calls, results, and returns with pinned native exit 197. Accepted CORE-053 takes non-escaping immutable scalar references across exact internal parameter/call boundaries through one whole-signature classifier, checked place binders, verifier provenance, typed pointer LLVM, and pinned native exit 211. Accepted CORE-054 adds explicit initialized owned local `Int`/`Float`/`Bool` reassignment across real branch/loop CFG through one shared assignment classifier, checked mutable-place/write identities, independent verification, and pinned native exit 227. CORE-055's candidate adds one non-escaping non-`Copy` mutable scalar alias, exact dereference writes, lexical release, shared classification, active-loan verification, typed pointer lowering, and a pending pinned exit-239 gate. Mutable-reference parameters/results, escaping provenance, stable reference ABI, general CFG ownership, NLL, drop, and lifetime inference remain absent; 16 broader semantic/lossy-shape Phase 5 tests remain quarantined | Add CFG-aware mutable/move/lifetime/destruction conformance without broadening semantics |
| Structs, arrays, enums, traits, and Match | CORE-043 provides an accepted bounded scalar-struct value slice. Accepted CORE-044 follows the framework's all-components-Copy rule for local aliases and exact by-value internal function transport through shared source classification, logical checked IR, verification, LLVM, and a pinned exit-63 public native gate. CORE-045 through CORE-047 add fixed Copy storage/transport and finite acyclic named aggregate graphs through pinned exits 77, 91, and 107. Accepted CORE-049 admits unique non-generic unit enums, exact payload-free construction, and explicit exhaustive scalar-result Match; accepted CORE-050 carries them through exact internal calls at exit 173. Accepted CORE-051 admits mixed unit and unary `Int`/`Float`/`Bool` variants, exact arm-local payload bindings, shared schema classification, checked extraction verification, and private tagged aggregate LLVM at exit 181. Accepted CORE-052 moves every admitted schema through generalized checked binders, call/return verification, aggregate SSA transport, and an exit-197 composed gate. Accepted CORE-053 composes that ADT/aggregate surface with a harder pointer-bearing reference-parameter boundary at exit 211; accepted CORE-054 composes explicit scalar mutation at exit 227 without broadening aggregate assignment; CORE-055's candidate composes local exclusive mutable aliasing at pending exit 239 without broadening function reference transport. Aggregate/generic/multi-field enums, enum containers/references, Option/Result Match, wildcard/guard/destructuring, direct nested/Bool arrays, cyclic aggregates, dynamic bounds, traits, and stable enum/aggregate/reference ABI remain open | Extend another hard ownership, trait, module, or layout class without broadening the accepted scalar-payload or reference semantics |
