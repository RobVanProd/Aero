...
| Structs, enums, traits, and Match | CORE-043 provides an accepted bounded scalar-struct value slice. The locally green CORE-044 candidate follows the framework's all-components-Copy rule for local aliases and exact by-value internal function transport through shared source classification, logical checked IR, verification, LLVM, and a pending exit-63 public native gate; non-Copy ownership, stable ABI, recursive aggregates, enums, traits, and Match remain open | Accept the pinned native gate, then extend aggregate composition or non-Copy ownership without broadening layout/ABI semantics before delivering an ADT/Match slice with the same proof depth |
