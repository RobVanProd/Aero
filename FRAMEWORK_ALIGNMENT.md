...
Local CAP-001 follows that corrected progression by selecting a real-program and
false-success blocker instead of another ownership topology. Before the candidate,
nonconstant fixed-array reads could form unchecked LLVM `inbounds` addresses and an
out-of-range program could falsely succeed. The candidate applies one ordered bounds
...
representative, full-root, external LLVM, and machine-verification evidence is green;
public Linux/Windows failure execution and protected acceptance remain pending. This
