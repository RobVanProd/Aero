...
`CORE-066` is now a green local candidate for the complementary loop-local class.
...
and the complete serialized root gate passes. Public LLVM/Clang 22 and all-eight
evidence remain pending, so this is not accepted capability yet. Outer-owner backedge/
exit joins, moved-target reinitialization, loop expressions/labels, and general CFG
ownership remain deliberately excluded.
...
| Ownership-based safety | Shallow move checks remain partial. CORE-048/053 through accepted CORE-065 establish bounded immutable/mutable whole-place ownership, internal reference transport, recursive finite CopyData composition, direct CopyData owner reassignment, owned enum transport/replacement, exact acyclic conditional joins, and independent enum-owner CFG consumption proof. Local CORE-066 candidate evidence additionally proves fresh per-iteration enum owners without transporting an outer moved owner. Mutable projections, reference results, escaping provenance, outer-owner loop joins, stable reference ABI, general CFG ownership, NLL, drop, lifetime inference, and memory-safety claims remain absent; 16 broader semantic/lossy-shape Phase 5 tests remain quarantined. | Publish and externally verify the exact CORE-066 candidate, then freeze another hard module, runtime, ownership, or execution slice |
