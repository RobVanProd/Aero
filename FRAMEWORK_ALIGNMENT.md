...
| Ownership-based safety | Shallow move checks remain partial. Accepted CORE-048 adds one real non-escaping immutable-reference subset for local `Int`/`Float`/`Bool` places through a shared classifier, checked alias-place provenance, verification, typed LLVM, all eight public checks, and pinned native exit 127. Mutable loans, reference ABI, NLL, drop, and general lifetime provenance remain absent; 16 broader semantic/lossy-shape Phase 5 tests remain quarantined | Add CFG-aware mutable/move/lifetime/destruction conformance without broadening semantics |
