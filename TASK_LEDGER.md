...
- Final closure: corrected exact six-record tree
  `1337945c51ecb8ffb9cb3aa1be33f4d3bd28dad4` and diff
  `861b5ec3f78fd39ae1fb2d297c10df6002584b71` received three fresh approvals with
  no P0-P3 findings after a prior snapshot was rejected for stale gate chronology.
  It was published unchanged as `226b7fbb89cfe9854b54d776ff5416bed516a670`.
  Compiler runs `30854853182` / `30854856449`, stable/nightly Rust `30854856190`,
  CodeQL `30854853829`, and aggregate `91823492290` all pass.
- Status: complete at public record closure `226b7fb`. Only parser-diagnostic start
  projection at the LSP boundary is accepted. R-009 remains HIGH/HIGH and OPEN;
  diagnostics/source spans remain PARTIAL and LSP remains EXPERIMENTAL.

## AUDIT-031 - Clean-head residual-risk re-ranking after CORE-024

- Task ID/date/owner: `AUDIT-031`, 2026-08-03, lead-owned read-only reconciliation
  with independent type/safety, IR/codegen, and backend/claim auditors.
- Observed behavior: `CORE-024` is complete at public closure `226b7fb`. Its selected
  parser-start UTF-16 adapter is controlled, but R-009 remains OPEN for token/AST
  end spans, recovery retention, and trustworthy end-to-end ranges. The complete
  remaining OPEN or PARTIALLY CONTROLLED set is R-002, R-004, R-005, R-006, R-007,
  R-009, R-010, R-011, R-012, R-013, and R-016. No next implementation is
  authorized.
- Hypothesis: a clean-head, delta-aware full-set ranking can identify the highest-
  severity bounded residual with frozen semantics, deterministic tests-first
  evidence, and at most two compiler phases, without repeating an accepted slice,
  inheriting a prior ordering, or treating records, annotations, flags, simulation,
  object emission, or editor coordinate projection as broader capability.
...
- Candidate boundaries and stops: R-002 excludes accepted numeric, selected binding/
  array, and non-entry Boolean helper contracts; unresolved entry/ABI and excluded
  types/shapes remain. R-004 needs ownership/provenance semantics. R-005 needs an
  unchecked-API compatibility policy or a distinct bounded trusted-path defect.
...
- Risks: stale residual accounting; ranking titles instead of active behavior;
  ...
  grammar, toolchain, or backend semantics; mistaking test or editor evidence for
...
- Final closure: superseded exact six-record snapshot `615c00b9`, tree `63c0b8ab`,
  diff `6ca586e6`, was rejected before publication because it still required a
  confirming gate after that gate had completed. Corrected snapshot
  `2ddf7bc969ff5bff795bb7a46ce91ac062070c45`, tree
  `6ec4c60975f6283ad88f553305b54e0c1ce49c0f`, and diff
  `4e1db1785f252d7e8ee26e745df9d5a96886a391` passed a third exact full gate and
  received three approvals with no P0-P3 findings. It was published unchanged as
  `0a940eadae5974abb11154c0e484f4178bfed144`. Compiler runs `30862783787` /
  `30862786131`, stable/nightly Rust `30862786150`, all three analyses in CodeQL
  `30862784231`, and aggregate `91848258218` all pass.
- Status: complete at public record closure `0a940ea`. Only the exact eligible known
  scalar/Void checked-admission arity phase-order guard is accepted. R-005 remains
  HIGH/CRITICAL and PARTIALLY CONTROLLED; no broader callable/type contract, matrix
  cell, or capability class is accepted.

## AUDIT-033 - Clean-head residual-risk re-ranking after CORE-026

- Task ID/date/owner: `AUDIT-033`, 2026-08-03, lead-owned read-only reconciliation
  with independent type/safety, IR/codegen, and backend/claim auditors.
- Observed behavior: `CORE-026` is complete at public closure `0a940ea`. Its selected
  eligible direct checked-AST arity mismatch now stops at Admission before raw IR,
  but R-005 remains PARTIALLY CONTROLLED for unchecked compatibility APIs, argument
  typing, other signatures/callables, and non-externally-verified publication paths.
  The complete remaining OPEN or PARTIALLY CONTROLLED set is R-002, R-004, R-005,
  R-006, R-007, R-009, R-010, R-011, R-012, R-013, and R-016. No next implementation
  is authorized.
- Hypothesis: a clean-head, delta-aware full-set ranking can identify the highest-
  severity bounded residual with frozen semantics, deterministic tests-first evidence,
  and at most two compiler phases, or stop explicitly, without repeating an accepted
  slice, inheriting a prior ordering, or treating rejection, records, annotations,
  flags, simulation, object emission, or editor projection as broader capability.
- Frozen audit semantics: the basis is the exact commit publishing this contract,
  only after its full local gate, three exact approvals, unchanged publication, and
...
- Candidate boundaries and stops: R-002 excludes accepted numeric, selected binding/
  ...
  accepted checked arity slice but retains unchecked-API policy and other bounded
  trusted-path defects. R-006 excludes direct modules/options controls but retains
  duplicated orchestration. R-007 requires real hardware execution/correctness
  evidence. R-009 excludes the parser-start adapter but retains spans/recovery.
  R-010 needs an authority/compatibility boundary. R-011 needs aggregate bounds/
  layout/execution semantics. R-012 needs an exact dormant-inventory population.
  R-013 excludes accepted status/wording/init controls but retains rollback, helper,
  maturity, and execution boundaries. R-016 needs a supported-toolchain policy.
- Allowed files/actions: during the audit, root and auditors are strictly read-only.
  After all findings are reconciled, only `TASK_LEDGER.md`, `DECISION_LOG.md`,
  `CURRENT_CAPABILITY_AUDIT.md`, `PROJECT_STATE.md`, `SPEC_IMPLEMENTATION_MATRIX.md`,
...
- Acceptance: this authorization passes exact `./tools/test.sh`, three exact-snapshot
  reviews, unchanged publication, and all eight public checks before audit work begins.
  Then three complete independent reports rank every residual and state findings,
  evidence, files changed, commands, test results, uncertainty, regression risks,
  commit SHA, and recommended next action. The lead reconciles one bounded selection
  or an explicit stop. Any test or implementation edit requires a later reviewed task
  contract and tests-first publication.
- Risks: stale residual accounting; ranking labels instead of active behavior;
  ...
  editor, object-emission, or simulation evidence for capability; or allowing an
  invalid program to IR/backend.
- Stop conditions: omitted residual; edit/test/probe/artifact/external query during
...
- Stop conditions: omitted residual; edit/test/probe/artifact/external query during
  ...
  package/release/registry, immutable evidence, history rewrite, destructive-system,
  or `master` action.
- Authorization evidence: exact reviewed snapshot
  `26c1eda074d871cabd1ceb8ec6808f6bec0d5209`, tree
  `f1baa4574b410b4f823e9ba8d1ae10772792188a`, and diff
  `1e8563ae0ac8ba95c6f1ece36d14e413eaf181bc` passed both initial and chronology-
  confirming full-local gates with 139/139 library and 149/149 binary tests plus
  every active integration and doc test, received three exact approvals, and was
  published unchanged as `45783af9b54277b83dd58fc9d6162163c451bbb3`.
  Compiler `30866227485` / `30866229553`, stable/nightly Rust `30866229554`, all
  three CodeQL analyses in `30866227939`, and aggregate `91858665436` all pass.
- Independent complete rankings: type/safety ranks R-002/R-009/R-012/R-005/R-011/
  R-004/R-013/R-010/R-006/R-016/R-007. IR/codegen ranks R-005/R-012/R-002/R-013/
  R-011/R-004/R-010/R-009/R-006/R-007/R-016. Backend/claim ranks R-005/R-002/
  R-011/R-016/R-013/R-012/R-006/R-009/R-010/R-004/R-007. All three used only
  static evidence at the immutable authorized head, changed no file, and ran no
  test, probe, artifact, or external query. R-012's exact dormant population is
  confirmed as 16 files/299 tests: fifteen unlinked `*_test.rs` files containing
  289 tests plus unlinked `pattern_matcher.rs` containing ten.
- Targeted reconciliation: all three auditors select one distinct R-002 false
  success over the R-005 runner-up. Exact
  `Statement::Let { type_annotation: Some(Type::Tuple(_)), value: None, .. }`
  currently passes direct semantics and checked admission; semantics fabricates
  `Ty::Int`, and raw generation can fabricate `ImmInt(0)`. This violates the hard
  unsupported-source-type fallback rule and reaches generation. The R-005 runner-up,
  a zero-argument call through a parameterized local closure alias, also reaches raw
  IR too early but remains fail-closed at mandatory verification before LLVM.
- Selection: DEC-033 permits only a separately reviewed tests-first two-phase
  containment for the exact valueless outer-tuple annotation. No audit finding by
  itself authorizes a test or implementation edit, closes R-002/R-005, or promotes
  any capability.
- Status: complete, read-only, and clean at authorized head `45783af`. `CORE-028`
  below is the separate frozen task contract; it must pass its own exact local gate,
  three reviews, unchanged publication, and all-eight public verification before any
  tests-first edit.

## CORE-028 - Reject uninitialized exact outer tuple binding annotations

- Task ID/date/owner: `CORE-028`, 2026-08-03, lead-owned two-phase R-002
  fail-closed containment under DEC-033.
- Observed behavior: `fn main() { let value: (int, float); }` succeeds through the
  public compiler. Direct semantics defaults every valueless binding to `Ty::Int`
  before binding insertion. Checked admission does nothing for a valueless `let`.
  Raw generation independently creates `(ImmInt(0), Ty::Int)`. Existing explicit
  CORE-025 quarantine assertions accept this exact uninitialized tuple form at both
  frontend boundaries; they are not support or compatibility evidence.
- Hypothesis: rejecting only the exact unsupported outer tuple annotation in
  semantics and independently in checked admission prevents the false `Int` fallback
  before binding insertion or generation without inventing tuple defaults, layout,
  ownership, ABI, or execution semantics and without affecting another annotation.
- Frozen semantics: match only
  `Statement::Let { type_annotation: Some(Type::Tuple(_)), value: None, .. }`.
  Direct semantics returns
  ``Error: Variable `NAME` uses an unsupported tuple type annotation for an uninitialized binding.``
  after existing same-scope duplicate-name rejection and before default `Ty::Int`,
  ownership processing, binding insertion, or symbol-table insertion. Checked
  admission independently returns
  ``checked IR binding `NAME` uses an unsupported tuple type annotation for an uninitialized binding``
  before raw generation; it does not invent duplicate-name validation. The public
  wrapper returns
  ``Semantic Analysis Error: Error: Variable `NAME` uses an unsupported tuple type annotation for an uninitialized binding.``
  Ordinary functions and every already traversed impl/generic-impl statement context
  use the same rule. With `value: None` there is no RHS-child precedence to change.
- Tests-first contract: after this authorization is public all-eight green, change
  only `src/compiler/tests/binding_type_contract_tests.rs`. Reverse the two explicit
  uninitialized-tuple acceptance assertions into exact direct semantic and checked-
  admission rejections; add exact public-pipeline, generic-impl semantic, and generic-
  impl checked-admission rejection evidence. Add a target-specific semantic
  precedence control for
  `fn main() { let value = 1; let value: (int, float); }` requiring the complete
  existing duplicate diagnostic
  ``Error: Variable `value` is already defined in this scope.`` Add direct semantic
  and checked-admission acceptance controls for `let value: int;` and for the
  explicitly excluded valueless non-tuple outer shapes
  `let value: [(int, float); 1];`, `let value: &(int, float);`, and
  `let value: Vec<(int, float)>;`. The latter are quarantine-preservation evidence,
  not support. The unchanged basis must fail only the selected aggregate regression
  while those exact precedence/outer-shape controls, existing use-before-
  initialization diagnostics, CORE-025 initialized tuple diagnostics/RHS-child
  precedence, and all unrelated tests stay green.
- Implementation boundary: only `src/compiler/src/semantic_analyzer.rs` and
  ...
  evidence. Add the smallest two guards at their existing `Statement::Let`
  boundaries. Do not change parser/AST, inference for any other declaration, raw IR
  generation, verifier, codegen, ABI, ownership, or backend behavior.
- Acceptance tests: the focused tuple binding contract is intentionally red at
  tests-first publication and green after implementation; direct semantics, direct
  checked admission, public `compile_program`, generic-impl semantics, and generic-
  impl checked admission match the complete exact diagnostics. Duplicate-name
  semantics remains first under the exact tuple-specific duplicate specimen. Exact
  scalar and valueless array/reference/generic outer-shape preservation specimens,
  initialized CORE-025 tuple controls, and valid LLVM remain unchanged. The exact
  repository-root `./tools/test.sh` gate, three exact-snapshot P0-P3 reviews,
  unchanged publication, and all eight public checks are required for authorization,
  tests-first, implementation, and final record closure as applicable.
- Exclusions: initialized bindings; tuple literals, projections, patterns, defaults,
  destructuring, parameters, returns, layout, or support; a tuple nested under an
  array, reference, generic, or other non-tuple outer annotation; all non-tuple
  valueless annotations; type-resolution redesign; later assignment semantics;
  unchecked API retirement; IR lowering, verifier, codegen, ABI, ownership, CPU,
  ROCm, CUDA, workflow/dependency, package/release/registry, benchmark, immutable
  claim-evidence, and capability/matrix changes.
- Risks: accidentally rejecting a nested-but-not-outer tuple shape; changing the
  established duplicate or initialized-RHS diagnostic order; adding tuple semantics
  rather than rejection; extending the guard to another unsupported annotation; or
  treating fail-closed containment as tuple capability.
- Stop conditions: any unresolved compatibility or diagnostic choice; need for tuple
  value/default/layout/ownership/ABI semantics; change beyond the one test file then
  the two named compiler phases; duplicate/RHS precedence change; another annotation
...
- Status: implementation accepted and public all-eight green. Superseded closure snapshot
  ...
  every active integration/doc test. Exact snapshot
  `f6305e18cd697e40d1262a7e0c1b1dc0a41fab4f`, tree
  `443aacdc1ac20eec7fc2c2d401f24c562568ce9c`, diff
  `93fce8ae5771f456f1943780952a831e1b8fc92b`, received three approvals and was
  published unchanged as `032d0d05f6fa4cfe3ac01e6add2b6fc4443cb338`.
  Compiler `30872236535` / `30872238993`, stable/nightly Rust `30872239003`, all
  three CodeQL analyses in `30872237025`, and aggregate `91876507154` all pass.
  CORE-028 is complete. R-002 remains HIGH/CRITICAL and PARTIALLY CONTROLLED. No
  tuple capability or matrix cell is promoted.
- Audit handoff: `AUDIT-035` below may re-rank the complete remaining OPEN/PARTIALLY
  CONTROLLED set only after its separate exact six-record authorization is reviewed,
  published unchanged, and all-eight green. It carries no implementation or
  capability authority.

## AUDIT-035 - Clean-head residual-risk re-ranking after CORE-028

- Task ID/date/owner: `AUDIT-035`, 2026-08-03, lead-owned read-only reconciliation
  with independent type/safety, IR/codegen, and backend/claim auditors.
- Observed behavior: `CORE-028` is complete at public closure `032d0d0`. The exact
  valueless outer `Type::Tuple(_)` binding now fails before fake `Int`, insertion, or
  generation at the two trusted boundaries, but this is containment rather than
  tuple support. The complete remaining OPEN or PARTIALLY CONTROLLED set is R-002,
  R-004, R-005, R-006, R-007, R-009, R-010, R-011, R-012, R-013, and R-016. No next
  implementation is authorized.
- Hypothesis: a clean-head, delta-aware full-set ranking can identify the highest-
  severity bounded residual with frozen semantics, deterministic tests-first
  evidence, and at most two compiler phases, or stop explicitly, without repeating
...
- Frozen audit semantics: the basis is the exact commit publishing this contract,
  ...
  including CORE-028, remains excluded. Any next task requires complete evidence,
  explicit exclusions, one separately reviewed frozen contract, and failing
  regression evidence first.
- Candidate boundaries and stops: R-002 excludes accepted numeric, selected binding/
  array, non-entry Boolean helper, initialized exact outer-tuple, and valueless exact
  outer-tuple rejection contracts; entry/ABI, lowercase string, custom/contextual/
  structural types, other uninitialized annotations, tuple annotations beneath non-
  tuple outer shapes, tuple support, and generic behavior remain. R-004 needs frozen
  ownership/provenance semantics. R-005 excludes accepted checked-arity containment;
  its known zero-argument parameterized-local-closure case remains verifier-contained
  before LLVM, while supplied-argument precedence, unchecked-API policy, other
  callable signatures, and trusted phase order remain unresolved. R-006 excludes
  direct-module/options controls but retains duplicated orchestration. R-007 requires
  real hardware execution/correctness evidence. R-009 excludes the parser-start
  adapter but retains spans/recovery. R-010 excludes authority notices but retains
  grammar compatibility, executable examples, and migration. R-011 needs aggregate
  bounds/layout/execution semantics. R-012 retains the confirmed 16-file/299-test
  disconnected population and needs truthful quality/activation classification.
  R-013 excludes accepted status/wording/init controls but retains rollback, helper,
  maturity, and execution boundaries. R-016 needs a supported-toolchain policy.
- Allowed files/actions: during the audit, root and auditors are strictly read-only.
  After all findings are reconciled, only `TASK_LEDGER.md`, `DECISION_LOG.md`,
...
- Acceptance: this authorization passes exact `./tools/test.sh`, three exact-snapshot
  ...
  selection or an explicit stop. Any test or implementation edit requires a later
  reviewed task contract and tests-first publication.
- Risks: stale residual accounting; inheriting an AUDIT-034 order; ranking labels
...
- Authorization evidence: first snapshot
  `b07adf20fc4eca192acf8ae3abff88674fb19a5c`, tree
  `fc524b74bb1a0ce0e29302a733af7710ea22deb7`, diff
  `4c64dc65237f5b68f8c634089e18eb1268c735ed`, was rejected at P2 before
  publication because two records made the absence of implementation/capability
  authority sound conditional on gate completion. The corrected contract states
  separately that gates unlock read-only ranking only and AUDIT-035 never grants
  implementation/capability authority. Corrected snapshot
  `5f8bdd4326882688c949decb897d4d68f1ff9dc9`, tree
  `ec2f779186037e74113a226040f85c7dc52fafd2`, diff
  `4c6defaec6f590e854f90e83a3be78e5b9f3d474`, fixed that P2 but was rejected at
  P3 before publication because Project State still instructed a future actor to run
...
- Authorization acceptance: exact snapshot
  `f4de8ef426d0fcc06a394adbcba5f03319b2107a`, tree
  `0b68565983b61e5891940aec88d4cba72fad2d3a`, diff
  `d3a9974b59e09d0cb307c76e159844b01cf1c6c5`, passed the fresh exact local gate
  and received three exact approvals. It was published unchanged as
  `987188fc265481d0de4c3021bcc5c3161aaeed12`. Compiler
  `30880025888` / `30880028697`, stable/nightly Rust `30880028653`, all three
  CodeQL analyses in `30880025866`, and aggregate `91899286217` pass.
- Independent complete rankings: type/safety ranks R-002/R-005/R-012/R-011/R-004/
  R-013/R-009/R-010/R-006/R-016/R-007. IR/codegen ranks R-002/R-005/R-011/R-012/
  R-013/R-006/R-009/R-004/R-010/R-016/R-007. Backend/claim ranks R-002/R-005/
  R-011/R-013/R-004/R-012/R-006/R-009/R-010/R-016/R-007. All three inspected
  the same immutable head, ranked every residual, excluded every accepted slice,
  changed no file, and ran no test, build, probe, artifact, hardware action, or
  external query.
- Initial candidate split: type/safety and IR/codegen selected the exact preserved
  `Type::Array(Type::Array(Type::Tuple(_), _), _)` valueless fallback. Backend/claim
  selected the separately preserved `Type::Reference(Type::Array(Type::Tuple(_),
  _), _)` form. Both reach fake semantic `Ty::Int`, skipped admission, verifier-
  valid raw integer zero, and trusted LLVM publication; R-005 ranks second because
  mandatory logical-IR verification already contains its wrong-arity closure call.
- Targeted static reconciliation: all three select the two-array candidate. It has
  identical trusted reach and two-phase feasibility but avoids reference mutability,
  universal-`Copy`, ownership, borrowing, and lifetime associations. Its two counts
  can be ignored mechanically on a valueless pre-IR rejection without deciding
  defaults, bounds, layout, mutation, or execution. The reference-array candidate
  and three-or-more array layers remain explicit preservation controls.
- Status: complete, read-only, and clean at public-green `987188fc`. AUDIT-037 made
  no risk-status, matrix, capability, test, or implementation change. The separately
  frozen CORE-031 contract below is the only selected next action.

## CORE-031 - Reject valueless immediate array-of-array-of-tuple annotations

- Task ID/date/owner: `CORE-031`, 2026-08-04, lead-owned two-phase R-002 fail-closed
  containment under DEC-038.
- Observed behavior: `fn main() { let value: [[(int, float); 1]; 1]; }` is an
  explicit accepted preservation control. Semantic analysis checks only outer tuple,
  immediate reference-to-tuple, and immediate array-to-tuple valueless annotations,
  then fabricates `Ty::Int` and inserts the binding. Checked admission applies the
  same three guards and skips every other valueless binding. Raw generation ignores
  the annotation and can fabricate verifier-valid `(ImmInt(0), Ty::Int)` that can
  reach trusted LLVM/CPU publication.
- Hypothesis: two matching nonrecursive guards can reject only a valueless annotation
  exactly shaped as `Type::Array(Type::Array(Type::Tuple(_), _), _)`, after existing
  semantic duplicate detection and before integer fallback, insertion, or raw
  generation, without defining nested-array/tuple defaults, values, bounds, layout,
  mutation, ABI, ownership, execution, or backend capability.
- Frozen semantics: the predicate is exactly `value.is_none()` plus outer
  `Type::Array(first, _)`, immediate `first: Type::Array(second, _)`, and immediate
  `second: Type::Tuple(_)`; both counts are irrelevant, including every zero/nonzero
  combination. Semantic duplicate-name diagnosis remains first. Semantics returns
  `Error: Variable \`{name}\` uses an unsupported tuple type annotation directly beneath two array layers for an uninitialized binding.`
  Checked admission independently returns
  `checked IR binding \`{name}\` uses an unsupported tuple type annotation directly beneath two array layers for an uninitialized binding`.
  Public compilation returns the semantic error before checked IR/backend work.
  Rejection is quarantine evidence only, not nested-array or tuple support.
- Tests-first contract: after this six-record authorization passes exact local,
  review, unchanged-publication, and public all-eight gates, only
  `src/compiler/tests/binding_type_contract_tests.rs` may first change. Add exact
  regression
  `valueless_immediate_array_of_array_of_tuple_annotation_fails_closed_before_integer_fallback`.
  It must reclassify only the existing deeper-array acceptance control and prove
  direct semantic and checked-admission rejection for outer/inner counts 0/0, 0/1,
  1/0, and 1/1, one public semantic rejection, and same-scope duplicate precedence.
  The focused test must publicly fail with exactly nine unexpected acceptances
  before any implementation edit.
- Preservation controls: exact CORE-028 outer-tuple, CORE-029 immediate reference-
  to-tuple, and CORE-030 immediate array-to-tuple diagnostics; scalar and nested
  scalar arrays; the still-accepted reference-array-tuple candidate; generic/Vec,
  reference-wrapped, and array-of-reference shapes; initialized exact target;
  three-or-more array layers around a tuple; raw compatibility behavior; and valid
  numeric-array LLVM output.
- Allowed implementation after separately reviewed public-red evidence: only
  `src/compiler/src/semantic_analyzer.rs` and
  `src/compiler/src/ir_generator.rs`. Add the smallest matching semantic and checked-
  admission guards. No recursive walker, raw generation, verifier, codegen, parser,
  AST, ABI, ownership, backend, workflow, dependency, package/release/registry,
  benchmark, immutable claim-evidence, or `master` change.
- Acceptance: tests-first yields one intentional binding-test failure containing
  exactly the nine frozen false acceptances while every preservation control and
  unrelated suite stays green. Implementation then passes the focused test, full
  binding aggregate, formatting, exact repository-root `./tools/test.sh`, three
  exact reviews, unchanged publication, and all eight public checks. R-002 remains
  HIGH/CRITICAL and PARTIALLY CONTROLLED; no matrix cell or capability class moves.
- Risks: recursive broadening; changing duplicate precedence or CORE-028/029/030
  diagnostics; rejecting the reference-array candidate, initialized forms, scalar
  arrays, wrappers, or third-and-deeper array shapes; treating counts as bounds or
  layout semantics; touching raw generation; or claiming aggregate support.
- Stop conditions: need for recursive traversal, parser/type representation changes,
  count-dependent behavior, another wrapper/depth outcome, initialized policy,
...
- Residual: Candidate B reference-array-tuple, initialized and third-plus-depth
  ...
  HIGH/CRITICAL and PARTIALLY CONTROLLED; no matrix cell or capability class moves.
- Status: complete at exact triple-approved public all-eight-green record closure
  `45696091d9ba10f97e1ce42b9372f330c3b4199b`, tree
  `480c35042a3db71c026f98281e527814735e4da0`, canonical diff
  `d682b0f6fcb55dc50e3c990488dad8994ddd0e81`. Compiler `30882630407` /
  `30882632698`, stable/nightly Rust `30882632696`, all three CodeQL analyses in
  `30882630822`, and aggregate `91907149874` pass. No capability, matrix cell, or
  risk status moved.

## AUDIT-038 - Clean-head full-risk reconciliation after CORE-031

- Task ID/date/owner: `AUDIT-038`, 2026-08-04, lead-owned read-only selection audit
  with three independent type/safety, IR/codegen, and backend/claim reviewers.
- Audit basis: exact clean public CORE-031 closure
  `45696091d9ba10f97e1ce42b9372f330c3b4199b`, tree
  `480c35042a3db71c026f98281e527814735e4da0`, canonical diff
  `d682b0f6fcb55dc50e3c990488dad8994ddd0e81`. It passes compiler
  `30882630407` / `30882632698`, stable/nightly Rust `30882632696`, all three
  CodeQL analyses in `30882630822`, and aggregate `91907149874` after three exact
  approvals.
- Observed behavior: CORE-031 closes only the exact valueless immediate array-of-
  array-of-tuple fallback. R-002/R-004/R-005/R-006/R-007/R-009/R-010/R-011/R-012/
  R-013/R-016 remain OPEN or PARTIALLY CONTROLLED with broader residuals. CORE-031's
  preserved reference-array candidate and all prior rankings are stale selection
  inputs rather than authority for another implementation.
- Hypothesis: three complete, independent, delta-aware static rankings from the
  exact clean public head can identify one distinct active residual with frozen
  semantics and deterministic tests-first feasibility within at most two compiler
  phases, or an explicit stop, without repeating an accepted slice or inventing
...
- Frozen audit method: each reviewer must independently inspect and rank the complete
  R-002/R-004/R-005/R-006/R-007/R-009/R-010/R-011/R-012/R-013/R-016 set from the
  exact audit basis. Each ranking must exclude every accepted sub-slice through
  CORE-031, cite file/symbol or line evidence for every rank, name one top candidate
  or stop, explain reachability and containment, state semantic decisions and phase
  count, propose an exact deterministic failing specimen plus preservation controls,
  and distinguish rejection, simulation, annotation, object emission, and hardware
  execution. The lead may reconcile rankings statically to one unanimous bounded
...
- Allowed files/actions: this authorization may change only `TASK_LEDGER.md`,
  ...
  No workflow, dependency, backend, package/release/registry, immutable claim-
  evidence, history, or `master` action is authorized.
- Acceptance: the authorization must pass exact repository-root `./tools/test.sh`,
  three exact reviews, unchanged publication, and all eight public checks before
  ranking begins. All three complete rankings must inspect the same immutable head
  and report the required nine evidence fields. Reconciliation may select at most
  one bounded residual or an explicit stop. AUDIT-038 never authorizes a regression
  test, implementation edit, capability promotion, risk-status change, or matrix
  movement; any later change requires its own reviewed six-record contract and
  public failing regression evidence first.
- Risks: omitting a residual or accepted-slice exclusion; inheriting Candidate B or
  another earlier order; conflating a verifier-contained defect with trusted
  publication; treating raw compatibility reachability as trusted admission;
  inventing annotation, ownership, aggregate, generic, ABI, or backend semantics;
  mistaking a rejection, test, LLVM text, helper simulation, or flag for implemented
  execution; or allowing an invalid program to IR/backend generation.
- Stop conditions: incomplete ranking; edit/test/probe/artifact/external query during
  the audit; more than two compiler phases; unresolved semantic or compatibility
  choice; hardware requirement; unsupported-source-type fallback; valid-output
  uncertainty; workflow/dependency, benchmark, package/release/registry, immutable
...
- Status: implementation remains complete and public all-eight green at `76a6e802`.
  Final additive correction `1ee9c71b555bec8066277cb9c64a7a7a2a3ff498`, parent
  `19f688a`, tree `d081988164fba75fcfe7af8788fbd010bb5a158d`, established
  PowerShell full-index canonical diff
  `7303da4793f01fd7d532f24030849761536835d0`, passed its fresh exact gate with
  139/139 library, 149/149 binary, 7/7 claim, and 22/22 binding tests, received
  three exact approvals, and was published unchanged. Compiler `30893527220` /
  `30893529999`, stable/nightly Rust `30893529992`, all three CodeQL analyses in
  `30893527445`, and aggregate `91941079083` pass. CORE-033 is accepted at this
  exact closure. R-002 remains HIGH/CRITICAL and PARTIALLY CONTROLLED; no matrix
  cell, capability class, or source behavior changed, and no published history was
  rewritten.

## AUDIT-040 - Clean-head full-risk reconciliation after CORE-033

- Task ID/date/owner: `AUDIT-040`, 2026-08-04, lead-owned read-only selection audit
  with three independent type/safety, IR/codegen, and backend/claim reviewers.
- Audit basis: exact clean public CORE-033 closure
  `1ee9c71b555bec8066277cb9c64a7a7a2a3ff498`, tree
  `d081988164fba75fcfe7af8788fbd010bb5a158d`, canonical diff
  `7303da4793f01fd7d532f24030849761536835d0`. It passed its exact local gate with
  139/139 library, 149/149 binary, 7/7 claim, and 22/22 binding tests, received
  three exact approvals, and passes compiler `30893527220` / `30893529999`,
  stable/nightly Rust `30893529992`, all three CodeQL analyses in `30893527445`,
  and aggregate `91941079083`.
- Observed behavior: CORE-033 closes only initialized exact nonrecursive
  `Array(Array(Tuple))` fallback after initializer validation. R-002/R-004/R-005/
  R-006/R-007/R-009/R-010/R-011/R-012/R-013/R-016 remain OPEN or PARTIALLY
  CONTROLLED with broader residuals. Candidate T, reference-array Candidate B,
  remaining deeper/wrapped/valueless forms, and every prior ranking are stale
  selection inputs rather than authority for another implementation.
- Hypothesis: three complete, independent, delta-aware static rankings from this
  exact clean public head can identify one distinct active residual with frozen
  semantics and deterministic tests-first feasibility within at most two compiler
  phases, or an explicit stop, without repeating an accepted slice or inventing
...
- Frozen audit method: each reviewer must independently inspect and rank the complete
  R-002/R-004/R-005/R-006/R-007/R-009/R-010/R-011/R-012/R-013/R-016 set from the
  exact basis. Each must exclude every accepted sub-slice through CORE-033, cite
  file/symbol or line evidence for every rank, name one top candidate or stop,
  explain trusted reachability and exact containment, state unresolved semantic
  decisions and phase count, propose one exact deterministic failing specimen plus
  preservation controls, and distinguish rejection, simulation, annotations, LLVM
  text, object emission, and hardware execution. The lead may reconcile statically
  to one unanimously bounded candidate or record a stop; no prior runner-up or
  preservation candidate is privileged.
- Frozen semantics: AUDIT-040 defines no language, type, ownership, aggregate,
  generic, ABI, verifier, lowering, runtime, tooling, backend, accelerator, or claim
  semantics. Candidate T/B/A and all other historical labels carry no selection
  priority. An unresolved compatibility choice is a stop, not implementation
  authority.
- Allowed files/actions: this authorization may change only `TASK_LEDGER.md`,
  `DECISION_LOG.md`, `CURRENT_CAPABILITY_AUDIT.md`, `PROJECT_STATE.md`,
  `SPEC_IMPLEMENTATION_MATRIX.md`, and `INITIAL_RISK_REGISTER.md`. Once its exact
  ...
  query. No workflow, dependency, backend, package/release/registry, immutable
  claim-evidence, history, or `master` action is authorized.
- Acceptance tests: first snapshot `c83ec3a` passed the exact repository-root
  `./tools/test.sh` gate with the Git Bash Cargo path inherited at exit 0 with
  139/139 library, 149/149 binary, 7/7 claim, and 22/22 binding tests. Focused claim
  and binding reruns also exited 0 at 7/7 and 22/22. The corrected authorization
  also passes a fresh exact full gate at exit 0 with 139/139 library, 149/149 binary,
  7/7 claim, and 22/22 binding tests. Three exact reviews, unchanged publication,
  and all eight public checks remain required before ranking begins.
  All three complete rankings must inspect the same immutable public head and report
  the nine required evidence fields. Reconciliation may select at most one bounded residual or an
  explicit stop. AUDIT-040 never authorizes a regression test, implementation edit,
  capability promotion, risk-status change, or matrix movement; any later change
  requires its own reviewed six-record contract and public failing regression
  evidence first.
- Risks: omitting a residual or accepted-slice exclusion; inheriting Candidate T/B/A
  or another earlier order; conflating verifier-contained or raw-API behavior with
  trusted publication; inventing annotation, ownership, aggregate, generic, ABI, or
  backend semantics; mistaking rejection, tests, LLVM text, helper simulation,
  annotations, flags, or object emission for implemented hardware execution; or
...
- Stop conditions: incomplete ranking; edit/test/probe/artifact/external query during
  ...
  uncertainty; workflow/dependency, benchmark, package/release/registry, immutable
  evidence, destructive-system, history-rewrite, or `master` action.
- Authorization review history: first snapshot
  `c83ec3a55ad341f7e0eabfe3c19741c1f6e7eb9d`, parent `1ee9c71`, tree
  `bb25e5289652a75d7585cc0a45b6799ed8a54df3`, canonical diff
  `c02f71e546963a4eb8ee132ae7f2210c6116a349`, changed exactly the six allowed
  control documents with 222 insertions/36 deletions and passed the exact gate with
  the counts above. Type/safety and backend/claim approved, but IR/codegen rejected
  at P1 because a late PROJECT_STATE chronology still said CORE-033 closure-only
  work remained and that its final correction must occur, contradicting accepted
  public closure `1ee9c71`. It was rejected before publication. The corrected tree
  converts that subsection to completed history; no source/test or ranking began.
- Authorization acceptance: corrected `7b9ed83b0663c4effcd63d2d9963e21b1416f54d`,
  parent `1ee9c71`, tree `8dbe975eff15e8b6741c2bd848b90cf1958cdcdf`,
  canonical diff `c4ba110a613064bf06a27ef943b3fc819c049b97`, received three
  exact approvals and was published unchanged. Compiler `30894708169` /
  `30894713332`, stable/nightly Rust `30894713411`, all three CodeQL analyses in
  `30894708736`, and aggregate `91944883143` pass.
- Independent complete rankings: type/safety ranked R-002/R-005/R-004/R-011/R-009/
  R-012/R-013/R-006/R-010/R-016/R-007 and selected valueless exact three-array
  tuple containment. IR/codegen ranked R-002/R-012/R-005/R-013/R-011/R-016/R-010/
  R-006/R-009/R-004/R-007 and selected initialized exact immediate reference-to-
  tuple containment. Backend/claim ranked R-011/R-005/R-002/R-004/R-006/R-013/
  R-009/R-010/R-007/R-012/R-016 and selected immediate nonnegative literal fixed-
  array bounds containment. Each inspected all eleven residuals on the immutable
  basis, excluded accepted slices, changed no file, and ran no test, probe, build,
  artifact, hardware action, or external query.
- Targeted comparison: type/safety and IR/codegen ranked reference/three-array/bounds;
  backend/claim ranked bounds/reference/three-array. The bounds path has the highest
  immediate backend consequence but conflicting records require runtime handling or
  permit compile-time/runtime error, so it remains stopped pending a separately
  frozen bounds policy. Exact three-array valueless rejection is bounded but has an
  arbitrary depth boundary, eight count combinations, and a larger preservation
  burden. The lead provisionally selected the initialized immediate reference shape
  on the two-to-one readiness result and absence of semantic invention. All three
  final compatibility reviews then approved that exact candidate, diagnostics,
  ordering, two-phase boundary, generic-context behavior, and preservation set.
- Status: complete, read-only, and clean at public-green `7b9ed83`. AUDIT-040 made
  no source, test, semantics, risk-status, matrix, capability, backend, or claim
  change. The separately frozen CORE-034 contract below is the only next action.

## CORE-034 - Reject initialized immediate reference-to-tuple annotations

- Task ID/date/owner: `CORE-034`, 2026-08-04, lead-owned two-phase R-002 fail-closed
  containment selected by AUDIT-040 final compatibility reconciliation.
- Observed behavior: active preservation rows at
  `src/compiler/tests/binding_type_contract_tests.rs:895-913` explicitly accept both
  `fn main() { let value: &(int, float) = 1; }` and its `&mut` form at semantics and
  checked admission. Reference and tuple annotations produce no selected binding
  contract, so the validated scalar RHS becomes effective `Ty::Int`; raw lowering
  ignores the annotation. Public compilation can therefore publish scalar LLVM
  after accepting a declared reference-to-tuple contract that has no value, layout,
  ABI, ownership, or lowering support.
- Hypothesis: after existing initializer child validation and initialized outer/
  one-array/two-array tuple diagnostics, one exact nonrecursive semantic guard and
  one exact checked-admission guard can reject only
  `Type::Reference(Type::Tuple(_), _)` for initialized bindings before mismatch,
  insertion, the generic-impl bypass, or raw generation. The reference mutability
  flag does not affect rejection. No reference/tuple capability follows.
- Frozen semantics and diagnostics: semantic duplicate-name rejection stays first.
  ...
  existing initialized outer-, one-array-, and two-array tuple diagnostics, then
  reject the exact target. Semantic diagnostic is `Error: Variable \`{name}\` uses an unsupported tuple type annotation directly beneath a reference for an initialized binding.` Checked diagnostic is
  `checked IR binding \`{name}\` uses an unsupported tuple type annotation directly beneath a reference for an initialized binding`; public compilation retains the
  existing `Semantic Analysis Error: ` prefix. Match both reference mutability flags
  without assigning mutability meaning.
- Context boundary: apply the guard wherever each named phase already traverses a
  binding, including direct function bodies, top-level statements, recursive block/
  if/while/for/loop statements, non-generic impl methods, generic impl methods, and
  semantic generic-function bodies. Checked generic functions retain the earlier
  `generic function IR is not admitted` rejection. Generic trait default bodies
  remain syntax-only; this task must not activate semantic or checked traversal for
  them.
- Tests-first contract: after this authorization is triple-approved and public
  ...
  change. Remove the two exact initialized rows from the existing preservation loop
  only by reclassifying them in one new aggregate test. Before implementation that
  aggregate must report exactly 30 unexpected acceptances: immutable/mutable direct
  semantic and checked boundaries (4); immutable/mutable public compilation (2);
  immutable/mutable top-level semantic and checked boundaries (4); immutable/mutable
  generic-impl semantic and checked traversal (4); immutable/mutable semantic
  generic-function traversal (2); and one immutable target in each of explicit
  block, if-then/else, while, for, loop, and non-generic impl contexts at semantic
  and checked boundaries (14). Checked generic-function outer rejection remains a
  green assertion. Focused result must be 0/1, binding aggregate 22/23 with only this
  test failing, and the exact full gate must retain 139/139 library, 149/149 binary,
  and 7/7 claim before reaching that intended failure.
- Required green precedence/preservation evidence: both duplicate-name cases; tuple,
  undefined, and Void RHS child errors; checked generic-function outer rejection;
  syntax-only generic trait defaults; initialized immutable/mutable scalar references;
  double references; reference-around-array, array-around-reference, generic,
  deeper, and wrapped forms; valueless direct immutable/mutable reference rejection;
  every accepted CORE-025/028/029/030/031/032/033 diagnostic and ordering; and valid
  numeric-array LLVM output.
- Later implementation boundary: only after separately reviewed, public expected-red
  evidence may implementation change `src/compiler/src/semantic_analyzer.rs` and
  `src/compiler/src/ir_generator.rs`, adding only the two exact guards. Parser, raw
  IR, verifier, codegen, LLVM verification, CLI orchestration, and backends must not
  change.
- Allowed authorization files/actions: before tests-first acceptance, only
  `TASK_LEDGER.md`, `DECISION_LOG.md`, `CURRENT_CAPABILITY_AUDIT.md`,
  `PROJECT_STATE.md`, `SPEC_IMPLEMENTATION_MATRIX.md`, and `INITIAL_RISK_REGISTER.md`
  may change. The prepared authorization's fresh exact repository-root
  `./tools/test.sh` gate exits 0 with 139/139 library, 149/149 binary, 7/7 claim,
  and 22/22 binding tests. It requires three exact reviews, unchanged publication,
  and all eight public checks before the tests-only change.
- Preservation: no reference/tuple value, mutability, ownership, lifetime, layout,
  ABI, coercion, default, lowering, execution, or compatibility semantics; no bounds
  decision; no raw compatibility API, valid-output, CPU, ROCm, or CUDA change; no
  matrix cell, capability class, or risk-status movement. Candidate V three-array
  valueless and Candidate L literal-bounds paths remain separate and unchanged.
- Risks: recursive matching; consuming valueless, double-reference, reference-array,
  array-reference, generic, deeper, or wrapped forms; placing rejection before
  duplicate/RHS/Void/existing diagnostics; applying it after the generic-impl bypass;
  accidentally traversing checked generic functions or trait defaults; adding a
  checked duplicate-binding rule; or treating rejection as reference/tuple support.
- Stop conditions: need for reference/tuple value, mutability, ownership, lifetime,
  layout, ABI, coercion, valid-output, bounds, or compatibility semantics; recursive
  classification; more than the two named compiler phases; raw/verifier/codegen/
  backend change; workflow/dependency/artifact/claim/history/`master` action; or any
  expected-red count/context that does not match the frozen matrix.
- First authorization snapshot `7d4d7ca`, tree `b633abbb`, canonical diff
  `a901f4dc`, passed its exact full local gate with 139/139 library, 149/149 binary,
...
- Closure acceptance: exact `d3811b00b15a594b9f3094f23cf61469db1133c8`, parent
  `a1ffeaec`, tree `c01088c482ee798202a71b4fe7dec859628ecf3e`, canonical diff
  `2799eb326c2e9097c53bad793b043293078a891a`, changed only the six control
  records with 153 insertions/19 deletions. Its fresh exact full local gate exits 0
  with 139/139 library, 149/149 binary, 7/7 claim, and 23/23 binding tests. Three
  exact reviews approved it, it was published unchanged, and compiler `30918433816`
  / `30918438945`, stable/nightly Rust `30918439169`, all three CodeQL analyses in
  `30918434204`, and aggregate `92022619964` pass.
- Status: CORE-034 is closed at exact public `d3811b00`. R-002 remains HIGH/CRITICAL
  and PARTIALLY CONTROLLED, R-011 bounds policy remains stopped, and no matrix,
  capability, backend, artifact, claim, history, or `master` state moved.

## AUDIT-041 - Clean-head full-risk reconciliation after CORE-034

- Task ID/date/owner: `AUDIT-041`, 2026-08-04, lead-owned read-only selection audit
  with three independent type/safety, IR/codegen, and backend/claim reviewers.
- Audit basis: exact clean public CORE-034 closure
  `d3811b00b15a594b9f3094f23cf61469db1133c8`, tree
  `c01088c482ee798202a71b4fe7dec859628ecf3e`, canonical diff
  `2799eb326c2e9097c53bad793b043293078a891a`. It passed the exact local gate with
  139/139 library, 149/149 binary, 7/7 claim, and 23/23 binding tests, received three
  exact approvals, and passes compiler `30918433816` / `30918438945`, stable/nightly
  Rust `30918439169`, all three CodeQL analyses in `30918434204`, and aggregate
  `92022619964`.
- Observed behavior: CORE-034 closes only initialized exact nonrecursive immediate
  reference-to-tuple fallback after initializer validation and existing tuple-shape
  diagnostics. R-002/R-004/R-005/R-006/R-007/R-009/R-010/R-011/R-012/R-013/R-016
  remain OPEN or PARTIALLY CONTROLLED with broader residuals. The prior three-array
  and bounds candidates and every earlier ranking are stale selection inputs rather
  than authority for another implementation.
- Hypothesis: three complete, independent, delta-aware static rankings from this
  exact clean public head can identify at most one distinct active residual with
  frozen semantics and deterministic tests-first feasibility within two compiler
  phases, or an explicit stop, without repeating an accepted slice or inventing
  language, ownership, aggregate, ABI, backend, or claim semantics.
- Frozen audit method: each reviewer must independently inspect and rank the complete
  R-002/R-004/R-005/R-006/R-007/R-009/R-010/R-011/R-012/R-013/R-016 set from the
  exact basis. Each must exclude every accepted sub-slice through CORE-034, cite
  file/symbol or line evidence for every rank, name one exact top candidate or stop,
  ...
  label, preservation row, or order is privileged.
- Frozen semantics: AUDIT-041 defines no language, type, ownership, aggregate,
  generic, ABI, verifier, lowering, runtime, tooling, backend, accelerator, or claim
  semantics. An unresolved semantic or compatibility choice is a stop, not
...
- Allowed files/actions: this authorization may change only `TASK_LEDGER.md`,
  ...
  query. No workflow, dependency, backend, package/release/registry, immutable
  claim-evidence, history, or `master` action is authorized.
- Acceptance tests: the prepared authorization's fresh exact repository-root
  `./tools/test.sh` gate exits 0 with 139/139 library, 149/149 binary, 7/7 claim,
  and 23/23 binding tests. It requires three exact reviews, unchanged publication,
  and all eight public checks before ranking begins. All three complete rankings
  must inspect the same immutable public head and report the nine evidence fields.
  Reconciliation may select at most one bounded residual or an explicit stop.
  AUDIT-041 never authorizes a regression test, implementation edit, capability
  promotion, risk-status change, or matrix movement; any later change requires its
  own reviewed six-record contract and public failing regression evidence first.
- Risks: omitting a residual or accepted-slice exclusion; inheriting the three-array,
  bounds, or another earlier order; conflating verifier-contained or raw-API behavior
  with trusted publication; inventing annotation, ownership, aggregate, generic,
  ABI, or backend semantics; mistaking rejection, tests, LLVM text, helper simulation,
  annotations, flags, or object emission for implemented hardware execution; or
  allowing an invalid program to reach IR/backend generation.
- Stop conditions: incomplete ranking; edit/test/probe/artifact/external query during
  the audit; more than two compiler phases; unresolved semantic or compatibility
