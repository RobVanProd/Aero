...
P1's public selector red is preserved, but its first admission proof exposed
the missing root-versus-function context described by CAP-033. The immediate
executable checkpoint is therefore the behavior-neutral context prerequisite;
P1 resumes only after that proof is accepted.
...
Execute `CAP-033-RESOLVED-SURFACE-CONTEXT` from accepted CAP-031. Extend each
CAP-030 surface observation with exact file-scope versus enclosing-function
context inside the existing one-pass finalizer, without changing public behavior.

Then resume CAP-032/P1 from the accepted CAP-033 head. P1 must consume the
context-bound CAP-030 surface witnesses and
