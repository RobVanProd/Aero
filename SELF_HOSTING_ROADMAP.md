...
| R2 — host byte input | **Authorized; red pending** | Deterministic whole-stream binary stdin ingestion through an Aero-owned EOF loop and accepted ByteBuffer | Empty/short/large input, partial prefix then failure, sticky EOF/I/O, binary sentinel bytes, Linux/Windows parity, no uninitialized bytes | Stdin bytes are not file/path I/O, text decoding, modules, or a production frontend |
...
|---|---|---|
| Runtime-sized source bytes | Accepted R1 supplies owned runtime-sized source storage, but no stdin/file byte ingestion. | CAP-039 closes only binary stdin through a new profile; file/path input remains separate |
...
| Compiler-safe selected surface | Accepted `exact-i32-byte-buffer-v0` adds only the bounded owner API while every earlier selector remains frozen. | CAP-039 adds a separate `exact-i32-byte-input-v0`; it must not change the accepted R1C selector |
| Input errors and typed failure | Accepted R1 maps private buffer failures into concrete `Result<int, int>`, but no runtime input failure contract exists. | R2 owns exact byte/EOF/I/O sentinels and their typed source mapping |
...
Implement and protect **CAP-039/R2 binary stdin** from the exact accepted R1
head. Add only the frozen runtime scalar read ABI, checked read instruction,
`stdin_read_byte() -> Result<int,int>`, new fail-closed
`exact-i32-byte-input-v0` selector, and public CPU stdin forwarding. The tracked
Aero loop must own EOF handling and ByteBuffer growth. Preserve R1, every prior
profile, parser/AST, and file/path semantics exactly; stop on a third compiler
authority or cross-platform byte/sentinel divergence.
